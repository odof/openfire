...
def product_price_update_before_done(self, forced_qty=None):
    ...
    for move in self.filtered(
            lambda move:
            move.with_company(move.company_id).product_id.cost_method == 'fifo' and float_is_zero(
                move.product_id.sudo().quantity_svl, precision_rounding=move.product_id.uom_id.rounding)):
        if move.product_id.categ_id.of_stock_update_standard_price:
