...
class OFPlanningInterventionLinkMixin(models.AbstractModel):
    ...
    equipment_id = fields.Many2one(comodel_name="of.equipment", string="Equipment", required=True)
    ...
    duration = fields.Float(related="task_id.duration", string="Duration", help="Duration in hours")
    ...
    def _get_changes_message_post(self, old_values, new_values):
        ...
        for field in new_values.keys():
            ...
            if field_type == "many2one":
                old_value = self.env[comodel_name].browse(old_values[field]).name_get()[0][1]
                new_value = self.env[comodel_name].browse(new_values[field]).name_get()[0][1]
            else:
        ...
        return (
            "<ul>"
            + "".join(f"<li>{data[field]['old_value']} {arrow} {data[field]['new_value']}</li>" for field in data)
            + "</ul>"
        )
