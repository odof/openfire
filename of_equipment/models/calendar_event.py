...
class CalendarEvent(models.Model):
    ...
    @api.onchange(
        # NOTE : Don't know why on `calendar.event` the `duration` field is not recomputed directly when linked
        # equipments are changed in the form view (as it is done on `of.service.request`).
        # That onchange is a workaround to force the recomputation of the duration in that case.
    ...
    def _compute_of_linked_equipment_ids(self):
        equipment_obj = self.env["of.equipment"].with_context(active_test=True)  # Ensure only active equipment are used
        events_use_equipment = self.filtered(
            lambda e: e.of_use_equipment and not e.of_linked_equipment_ids and (e.of_address_id or e.of_partner_id)
        )
        for event in events_use_equipment:
            domain = []

            # Only filter equipments on the address or partner if they are set
            ...
            if event.of_partner_id:
                domain[:0] = ["|"]
                domain += [("customer_id", "=", event.of_partner_id.id)]

            all_equipments = equipment_obj.search(domain) if domain else equipment_obj.browse()
            if not (
                equipments := all_equipments.filtered(
                    lambda e: e.site_address_id == event.of_address_id or e.customer_id == event.of_address_id
                )
            ):
                # If there is no equipment linked to the address we fallback to the partner
                equipments = all_equipments.filtered(lambda e: e.customer_id == event.of_partner_id)

            # And if only one equipment is found, we directly link it to the event otherwise we keep the
            # existing links (empty or not)
            event.of_linked_equipment_ids = (
                ...
                if len(equipments) == 1
                else event.of_linked_equipment_ids
            )
    ...
    def _report_get_equipment_links_print_mode_merged(self):
        ...
        """
        self.ensure_one()
        return self.of_linked_equipment_ids.filtered(  # First the equipment links without report template.
            lambda lnk: not lnk.equipment_report_tmpl_id
        ) + self.of_linked_equipment_ids.filtered(  # Then the equipment links in merged mode.
