...
class TestOFCalendarEventEquipmentLink(TestOFEquipmentCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()

        cls.calendar_event_link_obj = cls.env["of.calendar.event.equipment.link"]
    ...
    def test_04_calendar_event_available_equipment(self):
        """
        Test the availability of equipment linked to calendar events based on the event customer/address.
        """
        ...
        # We should be able to select all equipments linked to event customer, here Johnny Crash
        self.assertEqual(
            self.calendar_event_link_obj.new({"event_id": self.event.id}).equipment_ids_domain.ids,
            (
        ...
        self.event.of_partner_id = customer_one_equipment
        self.assertEqual(self.event.of_partner_id, customer_one_equipment)
        self.assertEqual(self.event.of_address_id, customer_one_equipment)
        ...
        # Set event address to Johnny Crash's address
        self.event.of_address_id = self.customer_johnny_crash.id
        self.assertEqual(self.event.of_partner_id, customer_one_equipment)
        self.assertEqual(self.event.of_address_id, self.customer_johnny_crash)
        ...
        self.event.of_partner_id = customer_no_equipment
        self.assertEqual(self.event.of_partner_id, customer_no_equipment)
        self.assertEqual(self.event.of_address_id, customer_no_equipment)

        # We should not be able to select any equipment
        self.assertFalse(self.calendar_event_link_obj.new({"event_id": self.event.id}).equipment_ids_domain)

    def test_05_event_switch_report_template_link(self):
        """
    ...
    def test_06_event_compute_duration(self):
    ...
    def test_11_event_report_equipment_links_printing_mode(self):
        """Test the equipment links printing mode for an event report.

        The test ensures that the equipment links are correctly found based on their sending report mode.
        """

        # Ensure the default sending report mode is set to "merged"
        self.assertEqual(self.event.of_template_id.default_equipment_report_tmpl_id.sending_report_mode, "merged")

        # Add equipment to the event
        self._event_wizard_add_equipment(
            [
        ...
        # Create a new report template with the sending report mode set to "attached" and change the report template
        # for one equipment to the attached mode
        eq_report_tmpl_maintenance_attached = self.env["of.equipment.intervention.report.template"].create(
            {
                "name": "Entretien new",
                "task_id": self.task_sweeping.id,
                "sending_report_mode": "attached",
            }
        )

        self.event.of_linked_equipment_ids[2].write(
            {"equipment_report_tmpl_id": eq_report_tmpl_maintenance_attached.id}
        )

        # Check linked equipment with sending report mode "merged"
        self.assertEqual(len(self.event._report_get_equipment_links_print_mode_merged()), 2)
        self.assertEqual(
            self.event._report_get_equipment_links_print_mode_merged(),
            self.event.of_linked_equipment_ids.filtered(
                lambda link: link.equipment_report_tmpl_id.sending_report_mode == "merged"
            ),
        )
        self.assertRecordValues(
            self.event._report_get_equipment_links_print_mode_merged(),
        ...
        # Check linked equipment to print in the report in attached mode
        self.assertEqual(len(self.event._report_get_equipment_links_print_mode_attached()), 1)
        self.assertEqual(
            self.event._report_get_equipment_links_print_mode_attached(),
            self.event.of_linked_equipment_ids.filtered(
                lambda link: link.equipment_report_tmpl_id.sending_report_mode == "attached"
            ),
        )
        self.assertRecordValues(
            self.event._report_get_equipment_links_print_mode_attached(),
        ...
        self.assertEqual(
            self.event._report_get_equipment_links_print_mode_attached(),
            self.event.of_linked_equipment_ids.filtered(
                lambda link: link not in self.event._report_get_equipment_links_print_mode_merged()
            ),
        )
