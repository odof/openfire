...
class OFWizvilleHistory(models.Model):
    ...
    def cron_do_export_wizville(self):
        ...
        For each file found, it calls the `action_button_put_file_wizville` method to handle the export.

        Returns:
            bool: Always returns True.
        """
        for file_to_integrate in self.search([("type", "=", "export"), ("is_export_done", "=", False)]):
            file_to_integrate.action_button_put_file_wizville()
        return True
    ...
    def _get_filenames_to_retrieve(self):
        ...
        """
        import_filename = self.env["ir.config_parameter"].get_param(
            "of.wizville.base.wizville_import_filename_prefix", default=""
        )
    ...
    def _download_and_process_files(self, sftp, filenames, remote_directory, rename_file=True):
        ...
        for sftpattr in sftp.listdir_attr():
            if hasattr(sftpattr, "filename"):
                ...
                if (
                    filename.endswith(f".{file_extension}")
                    and not filename.endswith(f".processed.{file_extension}")
        ...
        if potential_files:
            for filename in potential_files:
                ...
                if rename_file:
                    sftp.rename(
                        f"./{filename}", f"./{filename.replace(file_extension, f'.processed.{file_extension}')}"
                    )

                # Commit to avoid rollback issues on renamed files
                if not self.env.context.get("of_no_commit"):
    ...
    def get_file_wizville(self, rename_file=True):
        ...
        with self._get_sftp_connection() as sftp:
            ...
            remote_directory = (
                self.env["ir.config_parameter"].sudo().get_param("of.wizville.base.wizville_sftp_pickup_directory")
            )
    ...
    def _read_csv(self):
        ...
        Returns:
            iterator: An iterator over rows in the CSV file as dictionaries.
        """
        csv_data = base64.decodebytes(self.file)
        if not csv_data:
            raise UserError(_("No data found in the file."))

        encoding = self._get_file_encoding(csv_data)
        if encoding != "utf-8":
