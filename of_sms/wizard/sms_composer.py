...
class SendSMS(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if model == 'res.partner':
            numbers = self.env['res.partner'].browse(result.get('res_id')).get_mobile_numbers()
            if numbers:
    ...
    def _prepare_mass_sms_values(self, records):
        ...
        for record in results:
            record = dict(
                ...
                of_sender_id=self.of_sender_id.id,
                of_is_commercial=self.of_is_commercial,
                of_date_send=self.of_send_date,
            )
        return results

    def _action_send_sms_numbers(self):
        # TODO, ici OVH ne permet pas l'envoi par batch
        for number in self.sanitized_numbers.split(','):
            ...
            sms_id = self.env['sms.sms'].create(value_sms)
            self.env['sms.api']._send_sms_with_ovh_http(number, self.body, sms_id.id)
