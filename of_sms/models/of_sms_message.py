...
class OFSmsTemplate(models.Model):
    ...
    def render_template(self, template, model, res_id):
        ...
        user = self.env.user
        record = self.env[model].browse(res_id).with_context(self._context)

        # preprocess datetime fields (issue mako evaluating context)
        mako_variables = re.findall(r'\$\{[A-Za-z0-9_\.-]*\}',template)
        mako_variables_fields = [v.replace('${object.','').replace('}','') for v in mako_variables]
        mako_variables_fields_types = [type(record._fields[key]) for key in mako_variables_fields]
        mako_variables_readed = record.read(fields=mako_variables_fields)
        mako_variables_fields_vals = [mako_variables_readed[0][key] for key in mako_variables_fields]

        for i in range(len(mako_variables)):
            if mako_variables_fields_types[i].type == 'datetime': # le champ est datetime
                mako_variables_fields_vals[i] = fields.Datetime.to_string(fields.Datetime.context_timestamp(record,fields.Datetime.from_string(mako_variables_fields_vals[i])))
                template = template.replace(mako_variables[i],mako_variables_fields_vals[i])
