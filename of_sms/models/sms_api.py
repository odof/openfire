...
class SmsApi(models.AbstractModel):
    ...
    def _prepare_ovh_http_params(self, account, message, sms_id):
        params = super()._prepare_ovh_http_params(account, message, sms_id)
    ...
    def _send_sms_with_ovh_http(self, number, message, sms_id):
        ...
        r = requests.get(OVH_HTTP_ENDPOINT, params=self._prepare_ovh_http_params(account, message, sms_id), timeout=60)
        response = r.text
        if response[:2] != 'OK':
