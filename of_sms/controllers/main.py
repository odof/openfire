...
class OVHController(http.Controller):

    @http.route('/sms/ovh/mms/<attachment_id>/<string:filename>', type="http", auth="public", csrf=False)
    def sms_ovh_mms(self, attachment_id, filename):
        """Disable public access to MMS after OVH has fetched it"""
    ...
    """
    @http.route('/sms/ovh/receipt', type="http", auth="public", csrf=False)
    def sms_ovh_receipt(self, **kwargs):
        ...
        request.env['of.sms.gateway.ovh'].sudo().delivery_receipt(values['AccountSid'], values['MessageSid'])
        
        return "<Response></Response>"

    @http.route('/sms/ovh/receive', type="http", auth="public", csrf=False)
    def sms_ovh_receive(self, **kwargs):
        "" "Fetch the new message directly from OVH, don't trust posted data" ""
        ...
        account_id = request.env['sms.account'].sudo().search([('ovh_account_name','=', values['AccountSid'])])[0]
        ...
        if len(my_message) == 0:
            ...
            #inutilisé? twilio_gateway_id = request.env['sms.gateway'].sudo().search([('gateway_model_name', '=', 'sms.gateway.twilio')])
            ...
            if target['target_model'] == "res.partner":
                ...
                history_id = request.env['sms.message'].sudo().create({
                                                                'account_id': account_id.id,
                                                                'status_code': "RECEIVED",
                                                                'from_mobile': values['From'],
                                                                'to_mobile': values['To'],
                                                                'sms_gateway_message_id': values['MessageSid'],
                                                                'sms_content': values['Body'],
                                                                'direction':'I',
                ...
                for notify_partner in request.env['mail.followers'].sudo().search([
                                                                            ('res_model','=','crm.lead'),
