...
export class OFPartnerMap extends Component {
    ...
    async loadPartners(partner_ids){
        const partners = await this.orm.searchRead("res.partner",[['id','in',partner_ids]],["id","partner_latitude","partner_longitude"]);
    ...
    async loadPartner(partner){
        ...
        const partner_info = await this.orm.searchRead('res.partner',[['id','=',partner.id]],["id","display_name"]);
        if (partner_info){
    ...
    addMarker(partner){
        let marker = null;
        if (partner.icon){
            marker = L.marker([partner.partner_latitude, partner.partner_longitude],{icon: partner.icon}).addTo(this.leafletMap);
        } else {
            marker = L.marker([partner.partner_latitude, partner.partner_longitude]).addTo(this.leafletMap);
        }
    ...
    setup() {
        ...
        useEffect(
            () => {
                ...
                $(this.mapContainerRef.el).width($(this.mapContainerRef.el.parentElement.parentElement).width());
                this.partners.forEach(function(partner){
                    if (!self.leafletMap){
                        self.leafletMap = L.map(self.mapContainerRef.el).setView([partner.partner_latitude,partner.partner_longitude], 8);
