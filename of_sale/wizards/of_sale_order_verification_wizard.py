...
class OfSaleOrderVerification(models.TransientModel):
    ...
    type = fields.Selection(selection=[('margin', u'Contrôle de marge')], string="Type")
        ...
        if self.env['ir.values'].get_default('sale.config.settings', 'of_sale_order_margin_control'):
            ...
            if 'margin' not in skipped_types and not context.get('no_verif_margin', False):
                ...
                if (
                    ...
                    and int(order.of_marge_pc) < article_principal[0].product_id.categ_id.of_taux_marge
                ):
                    # les " et ' risque de faire planter le js, on les remplace par des espaces à la fin.
                    message = (
                        (
                            u"Le montant de marge de la commande %s est de %.2f%% alors que la catégorie %s de "
