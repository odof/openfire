...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    product_categ_id = fields.Many2one(
        'product.category',
        string=u"Catégorie des articles d'acompte",
    ...
    def _compute_of_nb_products(self):
        ...
        nb_products = (
            categ
            and self.env['product.product'].search([('categ_id', '=', categ.id), ('type', '=', 'service')], count=True)
            or 0
        )
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'percentage':
            ...
            result = super(
                SaleAdvancePaymentInv,
                self.with_context(of_acompte_sale_line_name=_("Advance: %s") % time.strftime('%d/%m/%Y')),
            ).create_invoices()
