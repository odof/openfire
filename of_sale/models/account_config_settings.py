...
class AccountConfigSettings(models.TransientModel):
    ...
    of_color_bg_section = fields.Char(
        ...
        help=u"Choisissez une couleur de fond pour les titres de section",
        default="#F0F0F0",
    )
    of_color_font = fields.Char(
        string="(OF) Couleur police titre section",
        help=u"Choisissez une couleur pour les titres de section",
        default="#000000",
    )
    ...
    def set_pdf_vt_pastille_defaults(self):
        return (
            self.env['ir.values'].sudo().set_default('account.config.settings', 'pdf_vt_pastille', self.pdf_vt_pastille)
        )
    ...
    def set_of_color_bg_section_defaults(self):
        return (
            self.env['ir.values']
            .sudo()
            .set_default('account.config.settings', 'of_color_bg_section', self.of_color_bg_section)
        )

    @api.multi
    def set_of_color_font_defaults(self):
        return self.env['ir.values'].sudo().set_default('account.config.settings', 'of_color_font', self.of_color_font)
    ...
    def set_of_validate_pickings(self):
        ...
        return (
            self.env['ir.values']
            .sudo()
            .set_default('account.config.settings', 'of_validate_pickings', self.of_validate_pickings)
        )
