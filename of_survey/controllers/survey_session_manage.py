...
class OFUserInputSession(http.Controller):
    def _fetch_from_token(self, survey_token):
        ...
        return request.env['of.survey.survey'].search([('access_token', '=', survey_token)])
    ...
    def _fetch_from_session_code(self, session_code):
        ...
        survey = request.env['of.survey.survey'].sudo().search([('session_code', '=', session_code)], limit=1)
        if not survey or survey.certification:
            return None, {'error': 'survey_wrong'}
        if survey.session_state in ['ready', 'in_progress']:
            return survey, None
        if request.env.user.has_group("of_survey.group_of_survey_user"):
    ...
    @http.route('/of_survey/session/manage/<string:survey_token>', type='http', auth='user', website=True)
    ...
    @http.route('/of_survey/session/results/<string:survey_token>', type='json', auth='user', website=True)
    def survey_session_results(self, survey_token, **kwargs):
        ...
        user_input_lines = request.env['of.survey.user_input.line'].search(
            [
                ('survey_id', '=', survey.id),
    ...
    @http.route('/of_survey/session/leaderboard/<string:survey_token>', type='json', auth='user', website=True)
    ...
    @http.route('/of_s', type='http', auth='public', website=True, sitemap=False)
