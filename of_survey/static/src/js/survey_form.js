odoo.define('of_survey.form', function(require) {
    ...
    publicWidget.registry.OFSurveyFormWidget = publicWidget.Widget.extend(OFSurveyPreloadImageMixin, {
        ...
        events: {
            ...
            'click button[name="save_create_attachment"]': '_onClickBtnSaveCreateAttachment',
        ...
        _onClickBtnAddAttachment: function(event) {
            ...
            $('#addAttachment .thumb').attr("src", "/web/static/img/placeholder.png");
        ...
        _saveAttachment: function(data){
            ...
            var $input = this.$('input.o_survey_upload_file');
            $input.attr('data-oe-data', JSON.stringify(this.images));
        },

        _udpateRenderImages: function(){
            var self = this;
            this.$(".form_images").html(qweb.render('of_survey.form_images', {
                images: this.images,
            }));
            ...
            this.$("a.delete").on("click", function(){
                ...
                $(".form_images").html(qweb.render('of_survey.form_images', {
                    images: self.images,
                }));
                var $input = self.$('input.o_survey_upload_file');
                $input.attr('data-oe-data', JSON.stringify(self.images));
            });
        },
        ...
        _onChangeChoiceItem: function(event) {
            ...
            if ($target.attr('type') === 'radio') {
                ...
                if ($matrixBtn.length > 0) {
                    ...
                    // Conditional display
                    if (this.options.questionsLayout !== 'page_per_question') {
                        var $question_id = newlySelectedAnswer.find(".o_survey_form_choice_item")[0].name;
                        var $answer_id = newlySelectedAnswer.find('input').val();
                        var $old_answer_id = previouslySelectedAnswer.find('input').val();

                        if (newlySelectedAnswer.find('.o_survey_form_choice_item_selected').length > 0) {
                            if (self.questions_answers.some(element => element.id == $question_id)) {
                                // On ajoute la réponse
                                var question = self.questions_answers.find(element => element.id == $question_id);
                                // on supprime l'ancienne réponse si elle a été déjà sélectionnée
                                question.answers = question.answers.filter(element => element != $old_answer_id);
                            ...
                            if (self.questions_answers.some(element => element.id == $question_id)) {
                                var question = self.questions_answers.find(element => element.id == $question_id);
                                question.answers = question.answers.filter(element => element != $answer_id);
                            }
                        }
                        ...
                        self._rpc({
                            ...
                            self._getInactiveConditionalQuestionIds().then(function(inactiveQuestions) {
                ...
                if ($matrixBtn.length > 0) {
                    ...
                    if (this.options.questionsLayout !== 'page_per_question') {
                        ...
                        var $question_id = $label.find(".o_survey_form_choice_item")[0].name;
        ...
        _validateForm: async function($form, formData) {
            ...
            var inactiveQuestionIds = this.options.sessionInProgress ? [] : await this._getInactiveConditionalQuestionIds();
            ...
            $form.find('[data-question-type]').each(function() {
                ...
                switch ($input.data('questionType')) {
                    ...
                    case 'multiple_choice':
                        ...
                        break;
                    case 'upload_file':
                        if (questionRequired && self.images.length == 0) {
                            errors[questionId] = constrErrorMsg;
                        }
                        break;
        ...
        _initBreadcrumb: function() {
            ...
            if ($breadcrumb.length) {
                this.OFSurveyBreadcrumbWidget = new publicWidget.registry.OFSurveyBreadcrumbWidget(this, {
