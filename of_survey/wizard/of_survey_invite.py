...
class OFSurveyInvite(models.TransientModel):
    _name = 'of.survey.invite'
    ...
    attachment_ids = fields.Many2many(
        ...
        relation='of_survey_mail_compose_message_ir_attachments_rel',
        column1='wizard_id',
        column2='attachment_id',
        string="Attachments",
    )
    # origin
    email_from = fields.Char(string="From", compute='_compute_email_from', readonly=False, store=True)
    author_id = fields.Many2one(
        comodel_name='res.partner', string="Author", index=True, ondelete='set null', default=_get_default_author
    )
    # recipients
    partner_ids = fields.Many2many(
        comodel_name='res.partner',
        relation='of_survey_invite_partner_ids',
        column1='invite_id',
        column2='partner_id',
        string="Recipients",
        domain="[ \
    ...
    existing_emails = fields.Text(
        string="Existing emails", compute='_compute_existing_emails', readonly=True, store=False
    )
    ...
    existing_text = fields.Text(string="Resend Comment", compute='_compute_existing_text', readonly=True, store=False)
    # technical info
    mail_server_id = fields.Many2one(comodel_name='ir.mail_server', string="Outgoing mail server")
    # survey
    survey_id = fields.Many2one(comodel_name='of.survey.survey', string="Survey", required=True)
    survey_start_url = fields.Char(string="Survey URL", compute='_compute_survey_start_url')
    ...
    def _compute_survey_start_url(self):
        for invite in self:
            invite.survey_start_url = (
                werkzeug.urls.url_join(invite.survey_id.get_base_url(), invite.survey_id.get_start_url())
                if invite.survey_id
                else False
            )
    ...
    def _compute_render_model(self):
        self.render_model = 'of.survey.user_input'
    ...
    def _onchange_emails(self):
        if self.emails and (self.survey_users_login_required and not self.survey_id.users_can_signup):
            raise UserError(
                _(
                    "This survey does not allow external people to participate. "
                    "You should create user accounts or update survey access mode accordingly."
                )
            )
        if not self.emails:
            return
        ...
        emails = list(set(emails_split.split(self.emails or "")))
        for email in emails:
            if email_check := tools.email_split_and_format(email):
    ...
    def _onchange_partner_ids(self):
        if self.survey_users_login_required and self.partner_ids and not self.survey_id.users_can_signup:
            if invalid_partners := self.env['res.partner'].search(
                [('user_ids', '=', False), ('id', 'in', self.partner_ids.ids)]
            ):
                raise UserError(
                    _(
                        "The following recipients have no user account: %s. You should create user accounts for "
                        "them or allow external signup in configuration.",
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('template_id') and not values.get('body') and not values.get('subject'):
    ...
    def _compute_subject(self):
        for invite in self:
            ...
            super(OFSurveyInvite, invite)._compute_subject()
    ...
    def _compute_body(self):
        for invite in self:
            ...
            if len(langs) == 1:
                invite = invite.with_context(lang=langs.pop())
            super(OFSurveyInvite, invite)._compute_body()
    ...
    def _prepare_answers(self, partners, emails):
        answers = self.env['of.survey.user_input']
        existing_answers = self.env['of.survey.user_input'].search(
            [
                '&',
        ...
        if existing_answers and self.existing_mode == 'resend':
            ...
            for partner_done in partners_done:
                answers |= next(
                    existing_answer
                    for existing_answer in existing_answers.sorted(lambda answer: answer.create_date, reverse=True)
    ...
    def _send_mail(self, answer):
        ...
        if email_layout_xmlid := self.env.context.get(
            ...
            template_ctx = {
                ...
                'model_description': self.env['ir.model']._get('of.survey.survey').display_name,
                'company': self.env.company,
            }
            if body := self.env['ir.qweb']._render(
                email_layout_xmlid,
                template_ctx,
    ...
    def action_invite(self):
        ...
        for email in emails_split.split(self.emails or ''):
            partner = False
            if email_normalized := tools.email_normalize(email):
            ...
            elif email_formatted := tools.email_split_and_format(email):
