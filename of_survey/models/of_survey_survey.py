...
class OFSurveySurvey(models.Model):
    ...
    def _get_default_session_code(self):
        ...
        for digits_count in range(4, 10):
            ...
            code_candidates = set(
                [str(random.randint(range_lower_bound, range_upper_bound)) for i in range(20)]  # nosec
            )
            colliding_codes = self.sudo().search_read([('session_code', 'in', list(code_candidates))], ['session_code'])
    ...
    users_can_go_back = fields.Boolean(
        string="Users can go back", help="If checked, users can go back to previous pages."
    )
    users_can_signup = fields.Boolean(string="Users can signup", compute='_compute_users_can_signup')
    # statistics
    answer_count = fields.Integer(string="Registered", compute='_compute_survey_statistic')
    answer_done_count = fields.Integer(string="Attempts", compute='_compute_survey_statistic')
    answer_score_avg = fields.Float(string="Avg Score (%)", compute='_compute_survey_statistic')
    ...
    success_count = fields.Integer(string="Success", compute='_compute_survey_statistic')
    success_ratio = fields.Integer(string="Success Ratio (%)", compute='_compute_survey_statistic')
    # scoring
    ...
    scoring_success_min = fields.Float(string="Required Score (%)", default=80.0)
    ...
    attempts_limit = fields.Integer(string="Number of attempts", default=1)
    is_time_limited = fields.Boolean(string="The survey is limited in time")
    time_limit = fields.Float(string="Time limit (minutes)", default=10)
    # certification
    certification = fields.Boolean(
        string="Is a Certification", compute='_compute_certification', readonly=False, store=True, precompute=True
    )
    certification_mail_template_id = fields.Many2one(
        comodel_name='mail.template',
        string="Certified Email Template",
        domain="[('model', '=', 'of.survey.user_input')]",
        help="Automated email sent to the user when they succeed the certification, containing their certification "
        "document.",
    )
    ...
    certification_give_badge = fields.Boolean(
        string="Give Badge", compute='_compute_certification_give_badge', readonly=False, store=True, copy=False
    )
    certification_badge_id = fields.Many2one(
        comodel_name='gamification.badge', string="Certification Badge", copy=False
    )
    ...
    session_code = fields.Char(
        ...
        copy=False,
        help="This code will be used by your attendees to reach your session. Feel free to customize it however "
        "you like!",
    )
    ...
    session_speed_rating = fields.Boolean(
        string="Reward quick answers", help="Attendees get more points if they answer quickly"
    )
    ...
    def _compute_survey_statistic(self):
        ...
        UserInput = self.env['of.survey.user_input']
    ...
    def _compute_is_attempts_limited(self):
        for survey in self:
            if not survey.is_attempts_limited or (survey.access_mode == 'public' and not survey.users_login_required):
    ...
    def _compute_session_link(self):
        for survey in self:
            if survey.session_code:
                survey.session_link = werkzeug.urls.url_join(survey.get_base_url(), f'/of_s/{survey.session_code}')
            else:
                survey.session_link = werkzeug.urls.url_join(survey.get_base_url(), survey.get_start_url())
    ...
    def _compute_certification_give_badge(self):
        for survey in self:
            if not survey.certification_give_badge or not survey.users_login_required or not survey.certification:
    ...
    def copy(self, default=None):
        ...
        answers_map = {
            ...
            for src_answer, dst_answer in zip(src.suggested_answer_ids, dst.suggested_answer_ids.sorted())
        }
    ...
    def _create_answer(
        ...
        for survey in self:
            ...
            if invite_token:
                ...
                answer_vals['invite_token'] = self.env['of.survey.user_input']._generate_invite_token()
    ...
    def _prepare_user_input_predefined_questions(self):
        ...
        for page in self.page_ids:
            ...
            elif page.random_questions_count > 0 and len(page.question_ids) > page.random_questions_count:
    ...
    def _get_number_of_attempts_lefts(self, partner, email, invite_token):
        ...
        return self.attempts_limit - self.env['of.survey.user_input'].search_count(domain)
    ...
    def _get_pages_and_questions_to_show(self):
        ...
        self.ensure_one()
        invalid_questions = self.env['of.survey.question']
    ...
    def _get_session_next_question(self, go_back):
        ...
        return self._get_next_page_or_question(
            most_voted_answers, self.session_question_id.id if self.session_question_id else 0, go_back=go_back
        )
    ...
    def _prepare_leaderboard_values(self):
        ...
        leaderboard = self.env['of.survey.user_input'].search_read(
        ...
        if (
            ...
            question_scores = {}
            input_lines = self.env['of.survey.user_input.line'].search_read(
                [
                ...
                leaderboard_item.update(
                    {
                        ...
                        'max_question_score': sum(
                            score
                            for score in self.session_question_id.suggested_answer_ids.mapped('answer_score')
                            if score > 0
                        )
                        or 1,
            ...
            leaderboard = sorted(leaderboard, key=lambda score: score['scoring_total'], reverse=True)
    ...
    def action_send_survey(self):
        ...
        # Ensure that this survey has at least one section with question(s), if question layout is
        # 'One page per section'.
        if self.questions_layout == 'page_per_section':
            if not self.page_ids:
                raise UserError(
                    _(
                        "You cannot send an invitation for a \"One page per section\" survey if the survey has "
                        "no sections."
                    )
                )
            if not self.page_ids.mapped('question_ids'):
                raise UserError(
                    _(
                        "You cannot send an invitation for a \"One page per section\" survey if the survey only "
                        "contains empty sections."
                    )
                )
        ...
        template = self.env.ref('of_survey.of_mail_template_user_input_invite', raise_if_not_found=False)
    ...
    def action_result_survey(self):
        ...
        return {
            ...
            'target': 'new',
            'url': f'/of_survey/results/{self.id}',
        }
    ...
    def action_survey_user_input(self):
        action = self.env['ir.actions.act_window']._for_xml_id('of_survey.action_survey_user_input')
        ctx = dict(self.env.context)
        ctx['search_default_survey_id'] = self.ids[0]
        action['context'] = ctx
        return action
    ...
    def action_survey_preview_certification_template(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'target': 'new',
            'url': f'/of_survey/{self.id}/certification_preview',
        }
    ...
    def action_open_session_manager(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'name': _("Open Session Manager"),
            'target': 'new',
            'url': f'/of_survey/session/manage/{self.access_token}',
        }
    ...
    def _prepare_statistics(self, user_input_lines=None):
        ...
        count_data_success = (
            self.env['of.survey.user_input']
            .sudo()
            ._read_group(user_input_domain, ['scoring_success', 'id:count_distinct'], ['scoring_success'])
        )
        completed_count = (
            self.env['of.survey.user_input'].sudo().search_count(user_input_domain + [('state', "=", "done")])
        )
        ...
        goal = self.env['gamification.goal.definition'].create(
            {
                ...
                'domain': f"['&', ('survey_id', '=', {self.id}), ('scoring_success', '=', True)]",
                'computation_mode': 'count',
                'display_mode': 'boolean',
                'model_id': self.env.ref('of_survey.model_of_survey_user_input').id,
                'condition': 'higher',
                'batch_mode': True,
                'batch_distinctive_field': self.env.ref('of_survey.field_of_survey_user_input__partner_id').id,
    ...
    def _handle_certification_badges(self, vals):
        if vals.get('certification_give_badge'):
            ...
            surveys_with_badge = self.filtered(
                lambda survey: survey.certification_badge_id and not survey.certification_badge_id.active
            )
