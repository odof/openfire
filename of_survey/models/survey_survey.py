...
class OFSurvey(models.Model):
    ...
    question_and_page_ids = fields.One2many(
        'of.survey.question', 'survey_id', string='Sections and Questions', copy=True
    )
    page_ids = fields.One2many('of.survey.question', string='Pages', compute="_compute_page_and_question_ids")
    question_ids = fields.One2many('of.survey.question', string='Questions', compute="_compute_page_and_question_ids")
    ...
    users_login_required = fields.Boolean(
        'Require Login', help="If checked, users have to login before answering even with a valid token."
    )
    ...
    answer_duration_avg = fields.Float(
        "Average Duration", compute="_compute_answer_duration_avg", help="Average duration of the survey (in hours)"
    )
    ...
    certification = fields.Boolean(
        'Is a Certification', compute='_compute_certification', readonly=False, store=True, precompute=True
    )
    ...
    certification_give_badge = fields.Boolean(
        'Give Badge', compute='_compute_certification_give_badge', readonly=False, store=True, copy=False
    )
    ...
    session_question_id = fields.Many2one(
        'of.survey.question', string="Current Question", copy=False, help="The current question of the survey session."
    )
    ...
    def _compute_background_image_url(self):
        ...
        for survey in self.filtered(lambda survey: survey.background_image and survey.access_token):
            survey.background_image_url = "/of_survey/%s/get_background_image" % survey.access_token
    ...
    def _compute_session_answer_count(self):
        ...
        for survey in self:
            answer_count = 0
            input_count = self.env['of.survey.user_input']._read_group(
                [
                    ('survey_id', '=', survey.id),
    ...
    def _compute_session_question_answer_count(self):
        ...
        for survey in self:
            answer_count = 0
            input_line_count = self.env['of.survey.user_input.line']._read_group(
                [
    ...
    def _compute_session_link(self):
        for survey in self:
            if survey.session_code:
                survey.session_link = werkzeug.urls.url_join(survey.get_base_url(), '/of_s/%s' % survey.session_code)
            else:
    ...
    def create(self, vals_list):
        surveys = super(OFSurvey, self).create(vals_list)
    ...
    def write(self, vals):
        result = super(OFSurvey, self).write(vals)
    ...
    def copy(self, default=None):
        ...
        """
        self.ensure_one()
        clone = super(OFSurvey, self).copy(default)
    ...
    def copy_data(self, default=None):
        ...
        return super(OFSurvey, self).copy_data(default)

    def toggle_active(self):
        super(OFSurvey, self).toggle_active()
    ...
    def _get_next_page_or_question(self, user_input, page_or_question_id, go_back=False):
        ...
        Question = self.env['of.survey.question']
        ...
        if survey.questions_layout == 'page_per_question':
            question_candidates = (
                pages_or_questions[0:current_page_index] if go_back else pages_or_questions[current_page_index + 1 :]
            )
    ...
    def action_send_survey(self):
        ...
        template = self.env.ref('of_survey.mail_template_user_input_invite_of', raise_if_not_found=False)
    ...
    def action_result_survey(self):
        ...
        return {
            ...
            'target': 'new',
            'url': '/of_survey/results/%s' % self.id,
        }
    ...
    def action_test_survey(self):
        ...
        return {
            ...
            'name': "Test Survey",
            'target': 'new',
            'url': '/of_survey/test/%s' % self.access_token,
        }
    ...
    def action_survey_preview_certification_template(self):
        self.ensure_one()
        return {'type': 'ir.actions.act_url', 'target': 'new', 'url': '/of_survey/%s/certification_preview' % (self.id)}
    ...
    def action_open_session_manager(self):
        ...
        return {
            ...
            'target': 'new',
            'url': '/of_survey/session/manage/%s' % self.access_token,
        }
    ...
    def get_start_url(self):
        return '/of_survey/start/%s' % self.access_token

    def get_start_short_url(self):
        """See controller method docstring for more details."""
        return '/of_s/%s' % self.access_token[:6]

    def get_print_url(self):
        return '/of_survey/print/%s' % self.access_token
