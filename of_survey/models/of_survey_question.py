...
class OFSurveyQuestion(models.Model):
    ...
    is_scored_question = fields.Boolean(
        ...
        store=True,
        copy=True,
        help="Include this question as part of quiz scoring. Requires an answer and answer score to be taken "
        "into account.",
    )
    # -- scoreable/answerable simple answer_types: numerical_box / date / datetime
    answer_numerical_box = fields.Float(
        string="Correct numerical answer", help="Correct number answer for this question."
    )
    answer_date = fields.Date(string="Correct date answer", help="Correct date answer for this question.")
    answer_datetime = fields.Datetime(
        string="Correct datetime answer", help="Correct date and time answer for this question."
    )
    answer_score = fields.Float(string="Score", help="Score value for a correct answer to this question.")
    # -- char_box
    ...
    matrix_subtype = fields.Selection(
        selection=[('simple', "One choice per row"), ('multiple', "Multiple choices per row")],
        string="Matrix Type",
        default='simple',
    )
    ...
    is_time_limited = fields.Boolean(
        string="The question is limited in time", help="Currently only supported for live sessions."
    )
    ...
    def _unlink_except_live_sessions_in_progress(self):
        if running_surveys := self.survey_id.filtered(lambda survey: survey.session_state == 'in_progress'):
            raise UserError(
                _(
                    "You cannot delete questions from surveys \"%(survey_names)s\""
                    " while live sessions are in progress.",
    ...
    def _prepare_statistics(self, user_input_lines):
        ...
        for question in self:
            ...
            if question.question_type in ['simple_choice', 'multiple_choice', 'matrix']:
                ...
                comment_line_ids = self.env['of.survey.user_input.line']
            ...
            question_data |= question._get_stats_summary_data(answer_lines)
    ...
    def _get_stats_data_answers(self, user_input_lines):
        ...
        if self.comment_count_as_answer:
            suggested_answers += [self.env['of.survey.question.answer']]

        count_data = dict.fromkeys(suggested_answers, 0)
        for line in user_input_lines:
            if line.suggested_answer_id in count_data or (line.value_char_box and self.comment_count_as_answer):
                count_data[line.suggested_answer_id] += 1

        table_data = [
            {
                'value': sug_answer.value if sug_answer else _("Other (see comments)"),
        ...
        graph_data = [
            {'text': sug_answer.value if sug_answer else _("Other (see comments)"), 'count': count_data[sug_answer]}
            for sug_answer in suggested_answers
        ]
        ...
        graph_data = [
            {
                ...
                'values': [{'text': row.value, 'count': count_data[(row, sug_answer)]} for row in matrix_rows],
            }
            for sug_answer in suggested_answers
        ]
    ...
    def _get_stats_summary_data(self, user_input_lines):
        ...
        if self.question_type in ['simple_choice', 'multiple_choice']:
            stats |= self._get_stats_summary_data_choice(user_input_lines)
        elif self.question_type == 'numerical_box':
            stats |= self._get_stats_summary_data_numerical(user_input_lines)
    ...
    def _get_stats_summary_data_choice(self, user_input_lines):
        right_inputs, partial_inputs = self.env['of.survey.user_input'], self.env['of.survey.user_input']
        ...
        if self.question_type == 'multiple_choice':
            for user_input, lines in tools.groupby(user_input_lines, operator.itemgetter('user_input_id')):
                user_input_answers = (
                    self.env['of.survey.user_input.line']
                    .concat(*lines)
    ...
    def _get_stats_summary_data_scored(self, user_input_lines):
        return {
            'common_lines': collections.Counter(
                user_input_lines.filtered(lambda line: not line.skipped).mapped(f'value_{self.question_type}')
            ).most_common(5),
