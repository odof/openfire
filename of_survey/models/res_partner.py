...
class ResPartner(models.Model):
    ...
    certifications_company_count = fields.Integer(
        string='Company Certifications Count', compute='_compute_certifications_company_count'
    )
    ...
    def _compute_certifications_count(self):
        read_group_res = (
            ...
            .sudo()
            ._read_group([('partner_id', 'in', self.ids), ('scoring_success', '=', True)], ['partner_id'], 'partner_id')
        )
    ...
    def action_view_certifications(self):
        action = self.env['ir.actions.actions']._for_xml_id('of_survey.res_partner_action_certifications')
