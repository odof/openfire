...
class OFSurveyQuestion(models.Model):
    ...
    survey_id = fields.Many2one('of.survey.survey', string='Survey', ondelete='cascade')
    scoring_type = fields.Selection(related='survey_id.scoring_type', string='Scoring Type', readonly=True)
    sequence = fields.Integer('Sequence', default=10)
    # page specific
    is_page = fields.Boolean('Is a page?')
    question_ids = fields.One2many('of.survey.question', string='Questions', compute="_compute_question_ids")
    ...
    # question specific
    page_id = fields.Many2one('of.survey.question', string='Page', compute="_compute_page_id", store=True)
    ...
    def _get_stats_data_answers(self, user_input_lines):
        ...
        graph_data = [
            {'text': _('Other (see comments)') if not sug_answer else sug_answer.value, 'count': count_data[sug_answer]}
            for sug_answer in suggested_answers
        ]
...
class OFSurveyQuestionAnswer(models.Model):
    ...
    question_id = fields.Many2one('of.survey.question', string='Question', ondelete='cascade')
    matrix_question_id = fields.Many2one('of.survey.question', string='Question (as matrix row)', ondelete='cascade')
    ...
    answer_score = fields.Float(
        'Score', help="A positive score indicates a correct choice; a negative or null score indicates a wrong answer"
    )
