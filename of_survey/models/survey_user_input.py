...
class OFSurveyUserInput(models.Model):
    ...
    last_displayed_page_id = fields.Many2one('of.survey.question', string='Last displayed question/page')
    # attempts management
    ...
    nickname = fields.Char(
        'Nickname', help="Attendee nickname, mainly used to identify them in the survey session leaderboard."
    )
    # questions / answers
    user_input_line_ids = fields.One2many('of.survey.user_input.line', 'user_input_id', string='Answers', copy=True)
    predefined_question_ids = fields.Many2many('of.survey.question', string='Predefined Questions', readonly=True)
    ...
    def create(self, vals_list):
        ...
        return super(OFSurveyUserInput, self).create(vals_list)
    ...
    def action_print_answers(self):
        ...
        return {
            ...
            'name': "View Answers",
            'target': 'self',
            'url': '/of_survey/print/%s?answer_token=%s' % (self.survey_id.access_token, self.access_token),
        }
...
class OFSurveyUserInputLine(models.Model):
    ...
    user_input_id = fields.Many2one(
        'of.survey.user_input', string='User Input', ondelete='cascade', required=True, index=True
    )
    survey_id = fields.Many2one(related='user_input_id.survey_id', string='Survey', store=True, readonly=False)
    question_id = fields.Many2one('of.survey.question', string='Question', ondelete='cascade', required=True)
    ...
    suggested_answer_id = fields.Many2one('of.survey.question.answer', string="Suggested answer")
    matrix_row_id = fields.Many2one('of.survey.question.answer', string="Row answer")
    # scoring
        ...
        for line in self:
            if line.answer_type == 'char_box':
                ...
                if line.matrix_row_id:
                    line.display_name = '%s: %s' % (line.suggested_answer_id.value, line.matrix_row_id.value)
                else:
    ...
    def create(self, vals_list):
        ...
        return super(OFSurveyUserInputLine, self).create(vals_list)
    ...
    def _get_answer_score_values(self, vals, compute_speed_score=True):
        ...
        if compute_speed_score and answer_score > 0:
            ...
            session_speed_rating = (
                user_input.exists() and user_input.is_session_answer and user_input.survey_id.session_speed_rating
            )
