...
class OfPlanningInterventionTemplate(models.Model):
    ...
    def name_get(self):
        ...
        for template in self:
            ...
            result.append((template.id, "%s%s%s" % ('' if meme_type else '(', template.name, '' if meme_type else ')')))
        return result
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if self._context.get('type_prio_id'):
            ...
            args = args or []
            res = (
                super(OfPlanningInterventionTemplate, self).name_search(
                    name, args + [['type_id', '=', type_id], ['type_id', '!=', False]], operator, limit
                )
                or []
            )
            ...
            if not limit or len(res) < limit:
                res += (
                    super(OfPlanningInterventionTemplate, self).name_search(
                        name, args + [['type_id', '=', False]], operator, limit and limit - len(res)
                    )
                    or []
                )
            if not limit or len(res) < limit:
...
class OFPlanningTache(models.Model):
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if rec_first != -1:
            res = (
                super(OFPlanningTache, self).name_search(name, args + [['recurrence', '=', rec_first]], operator, limit)
                or []
            )
            limit = limit - len(res)
            res += (
                super(OFPlanningTache, self).name_search(name, [['recurrence', '!=', rec_first]], operator, limit) or []
            )
            return res
...
class OFPlanningIntervention(models.Model):
    ...
    def _auto_init(self):
        ...
        if not exists:
            cr.execute(
                "UPDATE of_planning_intervention AS interv SET type_id = service.type_id "
    ...
    service_id = fields.Many2one(
        'of.service',
        string=u"Demande d'intervention",
    ...
    def onchange_template_id(self):
        ...
        if (
            ...
            and not self._context.get('of_import_service_lines')
    ...
    def _onchange_service_id(self):
        if self.service_id:
            ...
            if (
                ...
                and self._origin.service_id.note
    ...
    def write(self, vals):
        ...
        planif_avant = (
            state_interv in ('unfinished', 'cancel', 'postponed')
            and self.filtered(lambda i: i.state not in ('unfinished', 'cancel', 'postponed'))
            or False
        )
        # les cas 'cancel' -> 'during' -> 'confirm' ne peut pas arriver cf fonction find_interventions de of_mobile
        pas_planif_avant = (
            state_interv in ('draft', 'confirm')
            and self.filtered(lambda i: i.state in ('unfinished', 'cancel', 'postponed'))
            or False
        )
