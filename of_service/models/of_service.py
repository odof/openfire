...
WHERE os.partner_id = rp.id AND os.company_id IS NULL AND rp.company_id IS NOT NULL;
        ...
        cr.execute(
            "SELECT 1 FROM information_schema.columns " "WHERE table_name = 'of_service' AND column_name = 'recurrence'"
        )
        ...
        cr.execute("UPDATE of_service SET duree = 1, base_state = 'cancel' " "WHERE duree = 0")
    ...
    def function_set_date_next(self):
        ...
        if values:
            ...
            if values[0][0] == 'init_date_fin_contrat':
                ...
                cr.execute("UPDATE of_service " "SET date_fin_contrat = date_fin " "WHERE recurrence = 't'")
        else:
        ...
        services_avec_last = all_services.filtered(
            lambda s: (s.recurrence and s.date_last and (not s.date_next or s.date_last >= s.date_next))
        )
        for service in services_avec_last:
            ...
            cr.execute(
                ...
                "    date_next = %s "
                "WHERE id = %s",
                (date_next, date_next, service.id),
            )
    ...
    state = fields.Selection(
        ...
        u'État de planification',
        compute="_compute_state_poncrec",
        store=True,
    )
    ...
    secteur_tech_id = fields.Many2one(related='address_id.of_secteur_tech_id', readonly=True, store=True)
    ...
    tag_ids = fields.Many2many(
        ...
        relation='of_service_of_planning_tag_rel',
        column1='service_id',
        column2='tag_id',
    )
    ...
    jour_ids = fields.Many2many(
        ...
        'jour_id',
        string='Jours',
        default=lambda self: self._default_jours(),
        help=u"Jours de disponibilité du client.",
    )
    ...
    date_last = fields.Date(
        string=u'Dernier RDV',
        compute='_compute_durees',
        store=True,
        help=u"Date du dernier RDV en date. Ne prend pas en compte les RDVs annulés ni reportés",
    )  # Vue liste
    ...
    historique_interv_ids = fields.Many2many(
        string=u"Historique des interventions",
        comodel_name='of.planning.intervention',
        column1='of.service',
        column2='of.planning.intervention',
        relation='of_service_historique_interv_rel',
    ...
    priority = fields.Selection(
        [('0', 'Low'), ('1', 'Normal'), ('2', 'High')], string=u"Priorité", index=True, default='0'
    )
    ...
    def _compute_intervention_count(self):
        for service in self:
            service.intervention_count = len(
                service.intervention_ids.filtered(lambda r: r.state not in ('cancel', 'postponed'))
            )
    ...
    def _search_duree_restante(self, operator, operand):
        ...
        res = safe_eval(
            "services.filtered(lambda s: s.duree_restante %s %.2f)" % (operator, operand), {'services': services}
        )
        ...
        for service in self:
            ...
            if service.state == 'done' and interventions:
                ...
                service.spec_date = u"Prévue entre %s et %s" % (
                    format_date(service.date_next, lang),
                    format_date(service.date_fin, lang),
                )
    ...
    def _onchange_address_id(self):
        ...
        if self.address_id:
            ...
            company_choice = (
                self.env['ir.values'].get_default('of.intervention.settings', 'company_choice') or 'contact'
            )
    ...
    def create_invoice(self):
        ...
        for service in self:
            ...
            if not service.line_ids.filtered(lambda l: not l.invoice_line_ids):
                msgs.append(
                    u"Les lignes facturables de la demande d'intervention %s étant liées à des lignes de "
    ...
    def action_service_send(self):
        ...
        ctx = dict()
        ctx.update(
            {'default_model': 'of.service', 'default_res_id': self.ids[0], 'default_composition_mode': 'comment'}
        )
        try:
    ...
    def make_sale_order(self):
        ...
        if self.base_state != 'calculated':
            return self.env['of.popup.wizard'].popup_return(message=u"Cette demande d'intervention n'est pas validée.")

        # Ne pas créer de commande si pas de lignes
        if not self.line_ids:
            return self.env['of.popup.wizard'].popup_return(message=u"Cette demande d'intervention n'a aucune ligne.")
        ...
        if not self.fiscal_position_id:
            return self.env['of.popup.wizard'].popup_return(message=u"Veuillez renseigner une position fiscale.")
    ...
    def _prepare_invoice(self):
        ...
        if error:
            return (False, msg_erreur % (self.name, error))
        if not lines_data:
            return (False, msg_erreur % (self.name, u"Aucune ligne facturable."))
        ...
        journal_id = (
            self.env['account.invoice'].with_context(company_id=company.id).default_get(['journal_id'])['journal_id']
        )
        if not journal_id:
            return (
                False,
                msg_erreur
                % (self.name, u"Vous devez définir un journal des ventes pour cette société (%s)." % company.name),
            )
        invoice_data = {
        ...
        return (invoice_data, msg_succes % (self.name,))
    ...
    def get_state_poncrec_date(self, date_eval=fields.Date.today(), to_plan_avance=False):
        ...
        if self.base_state and self.base_state == 'calculated':
            ...
            date_fin_da = (
                self.date_fin and fields.Date.from_string(self.date_fin) or date_next_da + relativedelta(days=13)
            )
            if self.recurrence:
...
class OfServiceLine(models.Model):
    ...
    partner_id = fields.Many2one(comodel_name='res.partner', related='service_id.partner_id', readonly=True)
    ...
    invoice_line_ids = fields.One2many('account.invoice.line', 'of_service_line_id', string=u"Ligne de facturation")
    ...
    @api.depends(
        'invoice_line_ids', 'invoice_line_ids.invoice_id', 'invoice_line_ids.quantity', 'invoice_line_ids.invoice_id'
    )
