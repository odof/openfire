...
class OfPeriodePlanifiee(models.Model):
        ...
        for periode in self:
            if periode.premier_jour:
                ...
                if periode.type == 'semaine':
                    periode.name = date.strftime('S%W - %Y')
                if periode.type == 'mois':
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if self._context.get('from_task', False):
            ...
            res = tech.mapped('category_ids').filtered(lambda c: c.categorie_id.id == categ_id and c.temps_restant > 0).mapped('periode_id')
...
class OfPeriodePlanifieeTechnicien(models.Model):
    ...
    temps_de_travail = fields.Float(string="Temps de travail", required=True)  # widget="float_time"
    temps_restant_task = fields.Float(string=u'Temps restant à affecter (tâches)', compute="_get_temps_restant")  # widget="float_time"
    ...
    def write(self, vals):
        ...
        """
        if not vals.get('periode_id', True) and vals['periode_id'] == False: #and techniciens:
            del vals['periode_id']
    ...
    def _get_temps_restant(self):
        ...
        for tech in self:
            ...
            if not isinstance(tech.periode_id.id, models.NewId) and tech.user_id:
                activities = activity_obj.search([('date', '>=', tech.periode_id.premier_jour), ('date', '<=', tech.periode_id.dernier_jour), ('user_id', '=', tech.user_id.id)])
...
class OfPeriodePlanifieeCategory(models.Model):
    ...
    periode_id = fields.Many2one('of.periode.planifiee', string=u"Période", related="technicien_id.periode_id", readonly=True)
        ...
        for period_categ in self:
            ...
            if not isinstance(period_categ.periode_id.id, models.NewId):
                if period_categ.technicien_id and period_categ.categorie_id and period_categ.periode_id:
                    tasks = task_obj.search([('user_id', '=', period_categ.technicien_id.user_id.id),
                                             ('categ_id', '=', period_categ.categorie_id.id),
                                             ('of_periode_ids', 'in', [period_categ.periode_id.id])])
                    rest = sum(tasks.mapped('of_periode_time_ids').filtered(lambda t: t.periode_id.id == period_categ.periode_id.id).mapped('temps_affecte'))
...
class OfAffectationTache(models.Model):
    ...
    def _compute_temps(self):
        ...
        for time_periode in self:
            records = time_periode.tache_id.timesheet_ids.filtered(lambda ts: ts.date >= time_periode.periode_id.premier_jour \
                                                                              and ts.date <= time_periode.periode_id.dernier_jour)
...
class ProjectTask(models.Model):
    ...
    def _get_infos_on_periode(self, periode):
        """ Récupère les informations des périodes en fonction de l'utilisateur
        affecté a la tâche et de la catégorie de la tâche
        """
        vals = {}
        vals["periode_id"] = periode.id
        user_periode = periode.technicien_ids.filtered(lambda t: t.user_id.id == self.user_id.id)
        categ_user = user_periode.category_ids.filtered(lambda c: c.categorie_id.id == self.categ_id.id)
        vals["temps_affecte"] = self.of_time_left if self.of_time_left <= categ_user.temps_restant else categ_user.temps_restant
        return vals

    @api.onchange('planned_hours', 'of_periode_ids')
    def onchange_time(self):
        """ Change le temps affecter aux périodes si on ajoute une période
        ...
        time = self.planned_hours
        while records:
            first_date = min(records.mapped('name'))
            temp = records.filtered(lambda r: r.name == first_date)
            user_periode = temp.periode_id.technicien_ids.filtered(lambda t: t.user_id.id == self.user_id.id)
            categ_user = user_periode.category_ids.filtered(lambda c: c.categorie_id.id == self.categ_id.id)
            temp.temps_affecte = 0
            temp.temps_affecte = time if time <= categ_user.temps_restant else categ_user.temps_restant
            time -= temp.temps_affecte
            records -= temp
        return

    @api.onchange('of_periode_ids')
    def onchange_periode_ids(self):
        ...
        if self.of_periode_ids:
            ...
            to_add = affectation_obj
            already_used = self.of_periode_time_ids.mapped('periode_id').mapped('id')
            for periode in self.of_periode_ids:
                if periode.id in already_used:
                    continue
                to_add +=  affectation_obj.new(self._get_infos_on_periode(periode))
        self.of_periode_time_ids = self.of_periode_time_ids.filtered(lambda t: t.periode_id in self.of_periode_ids) + to_add
