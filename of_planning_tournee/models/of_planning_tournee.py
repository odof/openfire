...
class OfPlanningIntervention(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('date') or vals.get('employee_ids') or vals.get('state'):
            for rdvtech in self:
                date = vals.get('date', rdvtech.date)
                date_jour = isinstance(date, basestring) and date[:10] or date.strftime('%Y-%m-%d')
                employee_ids = set(rdvtech.employee_ids._ids)
                ...
                ajoute_ids = employee_ids - set(rdvtech.employee_ids.ids)
                retire_ids = set(rdvtech.employee_ids.ids) - employee_ids
                # sont concernés les ajouté et les retirés
                # ou tous si pas de modif employee_ids mais modif date ou state
                concerne_ids = (vals.get('employee_ids', False) and ajoute_ids | retire_ids) or employee_ids
                bloque_ids = planning_tournee_obj.search([('date', '=', date_jour),
                                                          ('employee_id', 'in', list(concerne_ids)),
                                                          ('is_bloque', '=', True)])
                if bloque_ids:
        ...
        for rdvtech in self:
            ...
            rdvtech_obj.remove_tournees(rdvtech.date_date, list(concerne_ids))
