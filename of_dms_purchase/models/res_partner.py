...
class ResPartner(models.Model):
    ...
    def create_purchases_directory(self):
        directory_obj = self.env["dms.directory"].with_context({"active_test": False})
        for partner in self:
            # on cherche s'il n'y a pas déjà un dossier purchases sur ce partenaire
            directory = directory_obj.search(
                [("parent_id", "=", partner.of_dms_directory_id.id), ("res_model", "=", "purchase.order")]
            )
            if not directory:
                value_directory = {
                    "name": "Purchases",
