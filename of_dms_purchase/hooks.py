# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

from odoo import SUPERUSER_ID, api


def _create_purchases_directory(cr, env):
    # pour chaque dossier partenaire, on ajoute un sous-dossier Sales
    partners = env["res.partner"].search([("user_ids", "=", False)])
    partners.create_purchases_directory()


def _create_purchases_files(cr, env):
    # pour chaque purchase order, on ajoute les pièces jointes qui peuvent déjà exister
    purchases = env["purchase.order"].search([])
    env["dms.file"].create_dms_files("purchase.order", purchases.ids, "partner_id", "Purchases")


def _create_virtual_file_reports(cr, env):
    # pour chaque company, on ajoute le rapport par défaut des fichiers virtuels
    value = {
        "model_id": env.ref("purchase.model_purchase_order").id,
        "report_id": env.ref("purchase.action_report_purchase_order").id,
    }
    cr.execute("select id from res_company")

    for company_id in cr.fetchall():
        value["company_id"] = company_id
        env["of.dms.virtual_file_report"].create(value)


def post_init_hook(cr, registry):
    env = api.Environment(cr, SUPERUSER_ID, {})

    _create_virtual_file_reports(cr, env)
    _create_purchases_directory(cr, env)
    _create_purchases_files(cr, env)
