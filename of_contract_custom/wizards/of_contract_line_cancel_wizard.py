...
class OFContractLineCancelWizard(models.TransientModel):
    ...
    def onchange_date_end(self):
        ...
        if self.contract_line_id.contract_id.date_end and self.date_end > self.contract_line_id.contract_id.date_end:
            raise UserError(
                u"Vous ne pouvez sélectionner une date de fin supérieure à la date de fin du contrat. "
                u"(%s)" % format_date(self.contract_line_id.contract_id.date_end, lang)
            )
        if self.contract_line_id.last_invoicing_date and self.date_end and \
        self.date_end < self.contract_line_id.last_invoicing_date:
            raise UserError(
                u"Vous ne pouvez sélectionner une date de fin antérieure à la date de dernière facturation. "
    ...
    def button_end(self):
        ...
        if not self.date_end or self.date_end > fields.Date.today():
            data['state'] = 'validated'
        self.contract_line_id.with_context(no_verification=True).write(data)
