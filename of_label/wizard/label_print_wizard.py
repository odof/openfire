...
class LabelPrintWizard(models.TransientModel):
    ...
    name = fields.Many2one('of_label.config', 'Label Size', required=True)
    ...
    brand_id = fields.Many2one('of_label.brand', 'Brand Name', required=True)
    ...
    def print_report(self):
        ...
        data = {
            'ids': self.ids,
            'model': 'of_label.config',
            'form': datas
        }
        return self.env['report'].get_action(self, 'of_label.report_label',
                                             data=data)
