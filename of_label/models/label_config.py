...
class LabelMain(models.Model):
    _name = 'of_label.brand'
    _rec_name = 'brand_name'

    brand_name = fields.Char('Name', size=64, index=True)
    label_config_ids = fields.One2many('of_label.config', 'label_main_id',
                                       'Label Config')


class LabelConfig(models.Model):
    _name = 'of_label.config'
    ...
    label_main_id = fields.Many2one('of_label.brand', 'Label')
