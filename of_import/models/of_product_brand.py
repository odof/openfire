    ...
    def _calculate_price_fields(
        ...
        for config_field, product_field, text in price_fields:
            for obj in (product, categ_config, self):
                ...
                if product_field != 'discount':
                    raise OfImportError(
                        _("No formula is provided for %(text)s of this item (brand to be configured: %(name)s)..")
                ...
                if price is not None:
                    ...
                    raise OfImportError(
                        _("No formula is provided for %(text)s of this item (brand to configure: %(name)s)..")
                        % {'text': text, 'name': self.name}
                    )
        return values
