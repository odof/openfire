...
class OfImport(models.Model):
    ...
    file_type = fields.Selection(selection=SELECT_EXTENSIONS, string="File Type", compute='_compute_file_type')
    ...
    added_count = fields.Integer(string="Added Count", readonly=True)
    updated_count = fields.Integer(string="Updated Count", readonly=True)
    failed_count = fields.Integer(string="Failed Count", readonly=True)
    ignored_count = fields.Integer(string="Ignored Count", readonly=True)
    ...
    def _compute_output_note(self):
        ...
        for record in self:
            ...
            if output_note:
                output_note = _("Fields available for import (column headers) :\n %(output_note)s") % {
    ...
    def action_import_line(self, line, file_fields, fields_odoo, i, model_data, duplicates, simulate):
        ...
        for file_field in file_fields:
            ...
            if file_field_norel in fields_odoo:
                ...
                if fields_odoo[file_field_norel]['required'] and (
                    ...
                    add_error(
                        _("line %(i)s : field %(field)s (%(file_field)s) empty but required. %(model)s not imported")
                        % {
                            'i': i,
        ...
        try:
            ...
            add_error(
                _("line %(i)s : %(e_name)s %(model)s not imported.")
                % {
                    'i': i,
                    'e_name': e.args[0],
                    'model': model_data['object_name'].capitalize(),
                ...
                try:
                    ...
                    messages = [
                        Command.create(
                            {
                                'type': 'info',
                                'message': _("Update %(model_name)s %(label_ref)s (line %(i)s)")
                                % {'model_name': model_data['object_name'], 'label_ref': label_ref, 'i': i},
                            },
                        )
                    ]
                ...
                try:
                    ...
                    messages = [
                        Command.create(
                            {
                                'type': 'info',
                                'message': _("Creation %(model_name)s %(label_ref)s (line %(i)s)")
                                % {
                                    'model_name': model_data['object_name'],
                                    'label_ref': label_ref,
                                    'i': i,
    ...
    def action_import_line_attachment(self, line, file_fields, fields_odoo, i, model_data, duplicates, simulate):
        ...
        try:
            if not any(file_path.startswith(p) for p in allowed_paths):
                add_error(
                    _("line %(i)s : You do not have right to access this file : %(file_path)s")
                    % {
                        'i': i,
                        'file_path': file_path,
                    }
                ...
                if not value:
                    add_error(
                        _("line %(i)s : the file is empty : %(file_path)s")
                        % {
                            'i': i,
                            'file_path': file_path,
                        }
            ...
            if path.exists(file_path):
                add_error(
                    _("line %(i)s : impossible to open the file : %(file_path)s") % {'i': i, 'file_path': file_path}
                )
            else:
        ...
        if res_field and res_obj is not False and res_field not in res_obj._fields:
            add_error(
                _("line %(i)s : field not found : %(res_model)s.%(res_field)s")
                % {
                    'i': i,
                    'res_model': res_model,
                    'res_field': res_field,
        ...
        if len(res_obj) == 1 and not res_field:
            ...
            if len(attachment) > 1:
                add_error(
                    _("line %(i)s : multiple attachments found : %(res_model)s - %(res_name)s : %(line_name)s")
                    % {
                        'i': i,
                        'res_model': res_model,
                        'res_name': res_name,
        ...
        if code == IMPORT_CREATION_CODE and not simulate:
            if res_field:
                ...
                try:
                    ...
                    messages = [
                        Command.create(
                            {
                                'type': 'info',
                                'message': _("Update of field %(res_field)s %(label_ref)s (line %(i)s)")
                                % {'i': i, 'res_field': res_field, 'label_ref': label_ref},
                            }
                        )
                    ]
                    ...
                    code = IMPORT_ERROR_CODE
                    add_error(
                        _("line %(i)s : Update failed %(res_model)s %(label_ref)s - Error : %(e)s")
                        % {
                            'i': i,
                            'res_model': res_model,
                            'label_ref': label_ref,
                ...
                try:
                    if attachment:
                        ...
                        messages = [
                            Command.create(
                                {
                                    'type': 'info',
                                    'message': _("Update attachment %(label_ref)s (line %(i)s)")
                                    % {
                                        'label_ref': label_ref,
                                        'i': i,
                                    },
                        ...
                        messages = [
                            Command.create(
                                {
                                    'type': 'info',
                                    'message': _("Create attachment %(label_ref)s (line %(i)s)")
                                    % {
                                        'label_ref': label_ref,
                                        'i': i,
                                    },
                    ...
                    code = IMPORT_ERROR_CODE
                    add_error(
                        _("line %(i)s : %(mode)s %(res_model)s %(label_ref)s - Error : %(e)s")
                        % {
                            'i': i,
            ...
            if file_field in fields_odoo and duplicates[file_field] > 1:
                import_errors += [
                    Command.create(
                        {
                            'type': 'error',
                            'message': _("The column %(file_field)s in the import file exist %(duplicates)s times")
                            % {
                                'file_field': file_field,
                                ...
                                Command.create(
                                    {
                                        ...
                                        'message': _(
                                            ...
                                            % {'brand_name': conf.brand_id.name, 'categ_origin': conf.categ_origin}
                                        ),
                                    },
                                )
                            ]
                    ...
                    Command.create(
                        {
                            ...
                            % {
                                'model_name': model_data['object_name'].capitalize(),
                                'key': key,
