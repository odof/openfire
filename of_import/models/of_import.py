...
class OfImport(models.Model):
    ...
    def _importer_ligne_attachment(self, ligne, champs_fichier, champs_odoo, i, model_data, doublons, simuler):
        ...
        if res_model not in self.env:
            erreur(u"Ligne %s : modèle non reconnu : " % (i, ligne['res_model']))
        else:
            res_name = ligne.get('res_id', '')
            res_obj = self.env[res_model].name_search(res_name, operator='=', limit=2)
            if not res_obj:
                erreur(u"Ligne %s : aucun résultat trouvé pour : " % (i, res_name))
            elif len(res_obj) > 1:
                erreur(u"Ligne %s : plusieurs résultats trouvés pour : " % (i, res_name))
            else:
