        ...
        for record in self:
            for field in (
                ...
                try:
                    ...
                    raise ValidationError(u"Une erreur s'est produite à la validation de la formule : "
                                          u"(champ \"%s\", formule \"%s\")\n\n"
                                          u"Erreur :\n%s" % (self._fields[field].string, code, e))
                if value and not isinstance(value, (int, long, float)):
                    raise ValidationError(u"Le format de retour de la fonction n'est pas celui attendu : "
                                          u"(champ \"%s\", formule \"%s\")\n\n"
                                          u"Cette erreur peut se produire si vous avez utilisé une virgule au lieu d'un"
                                          u" point comme séparateur de décimales." % (self._fields[field].string, code))
...
class OfImportProductCategConfig(models.Model):
    ...
    def _compute_product_ids(self):
        ...
        for categ in self:
            categ.product_ids = product_obj.search([('brand_id', '=', categ.brand_id.id),
...
class OFProductBrand(models.Model):
    ...
    def compute_product_categ(self, categ_name, product=None):
        ...
        categ_config = self.env['of.import.product.categ.config'].search([('brand_id', '=', self.id),
                                                                          ('categ_origin', '=', categ_name)])
            ...
            for obj in (product, categ_config, self):
                ...
                if product_field == 'remise':
                    ...
                    if price is not None:
                        ...
                        raise OfImportError(u"Aucune formule n'est renseignée pour %s de cet article "
                                            u"(marque à configurer : %s)." % (text, self.name))
                else:
                    raise OfImportError(u"Aucune formule n'est renseignée pour %s de cet article "
                                        u"(marque à configurer : %s)." % (text, self.name))
...
class OfImport(models.Model):
    ...
    type_import = fields.Selection(
        [('product.template', u'Articles'),
         ...
         ('of.service', u'Services OpenFire')], string=u"Type d'import", required=True)
    ...
    date = fields.Datetime(
        string='Date', required=True, default=lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    state = fields.Selection(
        [('brouillon', u'Brouillon'), ('importe', u'Importé'), ('annule', u'Annulé')], string=u'État',
        default='brouillon', readonly=True)
    ...
    import_success_ids = fields.One2many(
        comodel_name='of.import.message', inverse_name='import_id', string=u"Infos",
        domain=[('type', '=', 'info')], readonly=True)
    import_warning_ids = fields.One2many(
        comodel_name='of.import.message', inverse_name='import_id', string=u"Avertissements",
        domain=[('type', '=', 'warning')], readonly=True)
    import_error_ids = fields.One2many(
        comodel_name='of.import.message', inverse_name='import_id', string=u"Erreurs",
        domain=[('type', '=', 'error')], readonly=True)
    ...
    def _compute_sortie_note(self):
        ...
        for imp in self:
            sortie_note = ''
            for champ, valeur in sorted(self.get_champs_odoo(imp.type_import,
                                                             imp.lang_id.code or self.env.lang).items(),
    ...
    def get_champs_odoo(self, model='', lang=False):
        ...
        obj = self.env[model].fields_get(attributes=['field_description', 'required', 'type', 'relation',
                                                     'relation_field', 'translate', 'string'])
        for champ in obj:
    ...
    def _post_calcule_ligne(self, champs_fichier, ligne, model_data, res_objet, valeurs):
        ...
        if self.type_import == 'product.template':
            ...
            supplier_categ = valeurs.get('of_seller_product_category_name',
    ...
    def _importer_ligne(self, ligne, champs_fichier, champs_odoo, i, model_data, doublons, simuler):
        ...
        2 - Mise à jour de la référence de l'article avec le préfixe de la marque (si non déjà inclus,
            par exemple avec export+import)
        ...
        for champ_fichier in champs_fichier:
            ...
            champ_fichier_sansrel = (champ_fichier[0:champ_fichier.rfind('/')
                                     if champ_fichier.rfind('/') != -1
                                     else len(champ_fichier)].strip())
    ...
    #
                ...
                if champs_odoo[champ_fichier_sansrel]['requis']\
                        and (ligne[champ_fichier] == "#vide"
                             or (ligne[champ_fichier] == "" and not valeurs.get(champ_fichier_sansrel))):
    ...
    #
                ...
                # si est un many2one
                    ...
                    if model == 'product.template' and champ_fichier == 'categ_id':
                        ...
                        if ligne[champ_fichier] == '#vide':
                            ...
                            if res_objet:
                                ligne[champ_fichier] = '.'
                                valeurs['of_seller_product_category_name'] = res_objet.of_seller_product_category_name\
                                                                             or ''
                    if ligne[champ_fichier] == "#vide" and not champs_odoo[champ_fichier_sansrel]['requis']:
                        ...
                        # Si import de produit, la catégorie de produit peut avoir une correspondance
                            ...
                            if brand and simuler:
                                # Lors d'une simulation, les catégories manquantes sont ajoutées
                                # à la configuration de la marque
                                if not product_categ_config_obj.search([('brand_id', '=', brand.id),
                        ...
                        if len(res_ids) == 1:
                            ...
                            if model == 'res.partner' and champ_fichier == 'property_account_receivable_id'\
                                ...
                                if not simuler:
                                    valeurs[champ_fichier_sansrel] =\
                                        self.env[champs_odoo[champ_fichier_sansrel]['relation']]\
                                            .create({'name': ligne['name'],
                ...
                # Si est un one2many
                    ...
                    # Cas des fournisseurs pour les produits.
                    # Il y a un objet intermédiaire avec un enregistrement pour chaque produit.
                    ...
                    if model == 'product.template' and champ_fichier == 'seller_ids' and valeurs.get('brand_id'):
                        ...
                        if brand.partner_id:
                            if brand.partner_id.name.strip() != ligne[champ_fichier]:
                                erreur(u"Ligne %s : champ %s (%s) Le fournisseur choisi (%s) ne correspond pas "
                                       u"à celui de la marque %s (%s). %s non importé."
                                       % (i, champs_odoo[champ_fichier_sansrel]['description'], champ_fichier,
                                          ligne[champ_fichier], brand.name, brand.partner_id.name,
                                          model_data['nom_objet'].capitalize()))
                        else:
                            res_ids = self.env['res.partner'].search([('name', '=', ligne[champ_fichier]),
                                                                      ('supplier', '=', True)])
                            if len(res_ids) == 1:
                ...
                # On ne fait que prendre sa valeur sans traitement particulier
                    ...
                    if champs_odoo[champ_fichier_sansrel]['traduit'] and ligne[champ_fichier] != '':
                        ...
                        if lang != 'en_US':
                            ...
                            vals_trad[champ_fichier_sansrel] = ('' if ligne[champ_fichier] == "#vide"
                                                                else ligne[champ_fichier])
                    if champs_odoo[champ_fichier_sansrel]['langue']:
                        if ligne[champ_fichier] != '':
                            lang = champs_odoo[champ_fichier_sansrel]['langue']
                            vals_trad = valeurs_trad.setdefault(lang, {})
                            vals_trad[champ_fichier_sansrel[len(lang) + 2:]] = ('' if ligne[champ_fichier] == "#vide"
                                                                                else ligne[champ_fichier])
                    elif ligne[champ_fichier] == "#vide":
                        valeurs[champ_fichier_sansrel] = ''
                    elif ligne[champ_fichier] != '':
                        ...
                        if champ_fichier == model_data['champ_reference']:
                            if model == 'product.template':
                                # On ajoute (ou retire) à la référence d'un article le préfixe
                                # défini dans la marque associée
                                # Cette opération doit être réalisée après la détection de la marque
                                # mais avant la détection du produit associé
                                # (la combinaison préfixe+référence est la clef de recherche)
                                brand_id = valeurs.get('brand_id')
                                if brand_id:
                                    brand = self.env['of.product.brand'].browse(brand_id)
                                else:
    ...
    def importer(self, simuler=True):
        ...
        # On récupère la 1ère ligne du fichier (liste des champs)
        # pour vérifier si des champs existent en plusieurs exemplaires
        ...
        for champ_fichier in champs_fichier:
            ...
            if champ_relation:  # Si est défini, on le retire du nom du champ.
                champ_fichier = champ_fichier[0:(champ_fichier.rfind('/') if champ_fichier.rfind('/') != -1
                                                 else len(champ_fichier))].strip()
        ...
        for champ_fichier in doublons:
            # On affiche un message d'avertissement si le champ existe en plusieurs exemplaires
            # et si c'est un champ connu à importer
    ...
    #
        ...
        while True:
            ...
            if (nb_total + 1) % frequence_commit == 0:
                if model == 'product.template':
                    product_categ_configs = product_categ_config_obj\
                        .search([('id', 'not in', product_categ_config_ids)], order='brand_id, categ_origin')
            ...
            try:
                code, message = self._importer_ligne(ligne, champs_fichier, champs_odoo, i, model_data,
                                                     doublons, simuler)
        ...
        if model == 'product.template':
            product_categ_configs = product_categ_config_obj.search([('id', 'not in', product_categ_config_ids)],
                                                                    order='brand_id, categ_origin')
            if product_categ_configs:
                for config in product_categ_configs:
                    import_warning += [(0, 0, {'type': 'warning',
