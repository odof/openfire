...
odoo.define('of_planning_website.planning_website', function (require) {
    "use strict";
    ...
    $('.oe_website_sale').each(function () {
        ...
        $('.oe_cart').on('click', '.of_js_edit_address', function() {
            $(this).parent('div.one_kanban').find('form.hide').attr('action', '/rdv/nouveau/adresse/edition').submit();
        });
    ...
    $('.of_rdv_nouveau_tous_creneaux').on('click', '.of_js_change_creneau', function() {
        if (!$('body.editor_enable').length) { //allow to edit button text with editor
            var $old = $('.of_rdv_nouveau_tous_creneaux').find('.panel.border_primary');
            $old.find('.btn-ship').toggle();
...
odoo.define('of_planning_website.map_localisation', function (require) {
    'use strict';
