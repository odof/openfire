...
class OfPaiementEdi(models.Model):
    ...
    def _default_edi_service_line_ids(self):
        ...
        ouvertes et qui n'ont pas déjà été envoyées à l'affacturage"""
    ...
    def genere_fichier_lcr_service(self, edi_lignes):
        ...
        for edi_ligne in edi_lignes:
            ...
            if montant_du == 0:
                sortie += u"<b>DI non exigible</b> n° " + number_or_titre + u" de " + partner_display_name + u" [Montant total DI : " + str('%.2f' % edi_ligne.total_ttc).replace('.', ',') + u" euros]</li>\n"
                continue
            elif montant_du < 0:
                raise UserError(u"Erreur ! (#ED217)\n\nLa balance de la DI " + number_or_titre + u" de " + partner_display_name + u" est négative.\n\nVous ne pouvez payer par LCR que des DIs avec un solde positif.")
            else:
            ...
            if not rib:
                raise UserError(u"Erreur ! (#ED220)\n\nPas de compte bancaire trouvé pour " + partner_display_name + u".\n\nPour effectuer une LCR, un compte en banque doit être défini pour le client de chaque DI.")
            no_ligne = no_ligne + 1
            chaine += "0660"
            chaine += str(no_ligne).zfill(8)        # No de la ligne (no enregistrement sur 8 caractères)
            ...
            if not temp:
                ...
                if len(temp) == 14:   # C'est un n° SIRET. Le SIREN est les 9 premiers chiffres.
                    ...
                    raise UserError(u"Erreur ! (#ED230)\n\nLe n° SIREN de " + partner_display_name + u" dépasse 9 caractères.")
                chaine += temp.ljust(9, " ")
        ...
        if nb_service:  # Si des DIs sont à payer, on génère le fichier
            sortie = u"<p>Pour enregistrer l'opération et obtenir le fichier à transmettre à la banque, <b><u>vous devez valider le paiement des DIs</u></b>.</p>\n<p>Le fichier lettre change relevé (LCR) a été généré avec les éléments suivants :</p>\n" + sortie
    ...
    def genere_fichier_sepa_prev_service(self, edi_lignes):
        ...
        for edi_ligne in edi_lignes:
            ...
            rib = self._get_partner_rib(edi_ligne.service_id.partner_id)
            if not rib:
                raise UserError(u"Erreur ! (#ED436)\n\nPas de compte bancaire trouvé pour le client " + partner_display_name + u" (DI " + number_or_titre + u").\n\nPour effectuer une opération SEPA, un compte en banque doit être défini pour le client de chaque DI.")
            type_prev = rib.of_sepa_type_prev
            if not type_prev:
                raise UserError(u"Erreur ! (#ED431)\n\nLe champ \"Type de prélèvement SEPA\" n'a pas été configuré dans le compte en banque de " + partner_display_name + u" (DI " + number_or_titre + u").\n\nCe champ est obligatoire pour effectuer un prélèvement SEPA et se configure dans le compte en banque de la personne débitée.")
            if type_prev not in ('FRST', 'RCUR'):
                raise UserError(u"Erreur ! (#ED432)\n\nLe champ \"Type de prélèvement SEPA\" contient une valeur incorrecte dans le compte en banque de " + partner_display_name + u" (DI " + number_or_titre + u").\n\nVeuillez configurer ce champ à nouveau. Il se configure dans le compte en banque de la personne débitée.")
        ...
        for type_prev in services_par_type:
            ...
            for edi_ligne in services_par_type[type_prev]:
                ...
                # On vérifie que le montant à payer en fonction de l'échéancier n'est pas nul, sinon passe à la DI suivante
                if montant_du == 0:
                    sortie += u"<li><b>DI non exigible</b> n° " + number_or_titre + " de " + partner_display_name + u" [Montant total facture : " + str('%.2f' % edi_ligne[0].total_ttc).replace('.', ',') + u" euros]</li>\n"
                    continue
                elif montant_du < 0:
                    raise UserError(u"Erreur ! (#ED434)\n\nLa balance de la DI " + number_or_titre + " de " + partner_display_name + u" est négative.\n\nVous ne pouvez payer par prélèvement SEPA que des DIs avec un solde positif.")
    ...
    def action_enregistre_paiements_service(self):
        ...
        if not self.aff_bouton_paiement:
            raise UserError(u"Erreur ! (#ED303)\n\nVous avez modifié la liste des DI depuis la dernière génération du fichier de paiements.\n\nRegénérez une nouvelle fois le fichier avant d'effectuer une nouvelle validation des paiements.")

        # On regénère les fichiers LCR ou SEPA au cas où il y aurait eu des modifications sur les DIs/clients/émetteur depuis la génération.
        rib_rum_frst = False
        ...
        for paiement in paiements:
            ...
            if not payment:
                ...
                raise UserError(u"Erreur ! (#ED310)\n\nErreur création du paiement pour la DI n° " + (service.number or service.titre) + u" , client : " + service.partner_id.display_name + u".\n\nAucun paiement n'a été en conséquence validé.")
...
class OfPaiementEdiServiceLine(models.Model):
    ...
    service_id = fields.Many2one('of.service', string=u"Demande d'intervention", required=True, domain="[('state','not in',['cancel']')]")
