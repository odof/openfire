...
class AccountMoveLine(models.Model):
    ...
    def _compute_of_name(self):
        for line in self:
            ...
            if move.state == 'posted':
                updated_ref = (
                    f"{(move.partner_id.name or move.partner_id.parent_id.name or ''):.30} {move.name.strip()}"
                )
    ...
    def reconcile(self):
        ...
        for line in move_lines:
            ...
            if len(moves) == 1:
                name_infos = [f"{(moves.partner_id.name or moves.partner_id.parent_id.name or ''):.30}", moves.name]
