...
class OFParcInstalle(models.Model):
    ...
    def action_open_wizard_plan_intervention(self):
        ...
        if not self.client_id:
            raise UserError(
                _("There is no customer associated to this installed product. Please set one to plan an intervention."))
        if not self.client_id.geo_lat and not self.client_id.geo_lng:
