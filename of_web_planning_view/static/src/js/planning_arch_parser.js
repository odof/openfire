...
export class PlanningArchParser extends XMLParser{
    parse(arch) {
        ...
        this.visitXML(arch, (node) => {
            switch (node.tagName) {
                ...
                case "templates": {
                    popoverTemplate = node.querySelector("[t-name=planning-popover]") || null;
                    if (popoverTemplate) {
...
function getInfoFromRootNode(rootNode) {
    ...
    return {
        ...
        tripBarFields: attrs.trip_bar || null,
        scales,
        string: attrs.string || _t("Planning View").toString(),
