...
var PlanningView = View.extend({
     ...
     */
     willStart: function () {
        ...
        var company_filters_def = new Model('res.company').query(['id', 'name']).all();
        ...
        .then(
            function (write, create, excluded, emp_ids, attendee_mode, company_filters, now_company_id, hauteur_fixe,
              duree_to_px) {
                ...
                for (var i=0; i<company_filters.length; i++) {
                    company_fil = company_filters[i]
                    self.company_ids_indexes[company_fil.id] = i;
                ...
                for (var i=0; i<attendees.length; i++) {
                    ...
                    if (self.attendee_mode == 'tech' && a['of_est_intervenant'] ||
                      self.attendee_mode == 'com' && a['of_est_commercial'] || self.attendee_mode == 'comtech') {
                        if (!a['company_id'] || !self.now_company_id || a['company_id'][0] == self.now_company_id) {
                   ...
                   //console.log("events: ",events,self.fields_keys);
                    ...
                    for (var i=0; i<events.length; i++) {
                        ...
                        for (var j in event_res_ids) {
                            ...
                            // ne pas ajouter le planning_record pour les lignes à ne pas montrer
    ...
    apply_extra_filters: function() {
        ...
        for (var i in this.all_filters) {
            if (!this.all_filters[i].company_id || this.now_company_id == 0
