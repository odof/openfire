...
class OfPlanifCreneau(models.TransientModel):
    ...
    secteur_id = fields.Many2one('of.secteur', string="Secteur",
                                 help=u"laisser vide pour ne pas restreindre à un secteur en particulier")
    ...
    pre_tache_ids = fields.Many2many('of.planning.tache', string=u"Tâches",
                                     help=u"Remplir pour restreindre la recherche à certaines tâches")
    ignorer_duree = fields.Boolean(string=u"Ignorer durée",
                                   help=u"Cochez pour proposer aussi les interventions plus longues que le créneau")
    ...
    def _compute_num_jour(self):
        for creneau in self:
            if creneau.date_creneau:
                ...
                creneau.num_jour = self.env['of.jours'].search([('numero', '=', date_creneau_da.isoweekday())],
                                                               limit=1).id
    ...
    def button_confirm_next(self):
        ...
        if same_day:  # réinitialiser les champs du wizard pour lancer une nouvelle recherche
            date_fin_interv_locale_dt = fields.Datetime.context_timestamp(self, fields.Datetime.from_string(
                intervention.date_deadline))
            ...
            for i in range(len(creneaux)):
                ...
                if creneau[0] <= interv_heure_fin <= creneau[1]:  # trouvé!
                    if float_compare(interv_heure_fin, creneau[1],
                                     5) == 0:  # le créneau a été entièrement rempli: on le supprime
...
class OfPlanifCreneauProp(models.TransientModel):
    ...
    _order = "creneau_id DESC, selected DESC, priorite DESC, distance_arrondi_order, date_next, date_fin, distance_order"
    ...
    distance_oiseau_prec = fields.Float(string=u'Distance du précédent', digits=(5, 5),
                                        compute="_compute_distance_oiseau", help=u"À vol d'oiseau")
    distance_oiseau_suiv = fields.Float(string=u'Distance du suivant', digits=(5, 5),
                                        compute="_compute_distance_oiseau", help=u"À vol d'oiseau")
    ...
    def _compute_distance_oiseau(self):
        for a_planifier in self:
            ...
            if a_planifier.creneau_id.geo_lat_prec == 0.0 or a_planifier.creneau_id.geo_lng_prec == 0.0:
                ...
                a_planifier.distance_oiseau_prec = voloiseau(a_planifier.geo_lat, a_planifier.geo_lng,
                                                             a_planifier.creneau_id.geo_lat_prec,
                                                             a_planifier.creneau_id.geo_lng_prec)
            if a_planifier.creneau_id.geo_lat_suiv == 0.0 or a_planifier.creneau_id.geo_lng_suiv == 0.0:
                a_planifier.distance_oiseau_suiv = -1
            else:
                a_planifier.distance_oiseau_suiv = voloiseau(a_planifier.geo_lat, a_planifier.geo_lng,
                                                             a_planifier.creneau_id.geo_lat_suiv,
