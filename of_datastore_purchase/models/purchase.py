...
class PurchaseOrder(models.Model):
    ...
    @api.depends('picking_ids.of_datastore_anomalie')
    ...
    def get_datastore_order_values(self, base_vals):
        ...
        if self.of_datastore_dropshipping:
            values['of_datastore_purchase_id'] = self.id
        return values
...
class PurchaseOrderLine(models.Model):
    ...
    def get_datastore_move_id(self, datastore_move_id, datastore_picking_id):
        ...
        if len(moves) > 1:
            return False, False
        moves.write({'of_datastore_move_id': datastore_move_id})
        if moves.picking_id:
            moves.picking_id.write({'of_datastore_id': datastore_picking_id})
        return moves.id, moves.picking_id.id or False
