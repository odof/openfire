...
class OFDatastorePurchase(models.Model):
    ...
    datastore_id = fields.Char(string=u"Identifiant chez le fournisseur", required=False)
...
class OFDatastorePurchaseCompany(models.Model):
    ...
    def _auto_init(self):
        ...
        if not exists:
            datastores = self.env['of.datastore.purchase'].search([])
            datastore_company_obj = self.env['of.datastore.purchase.company']
            for datastore in datastores:
                datastore_company_obj.create({
                    'parent_id': datastore.id,
