...
class ProductTemplate(models.Model):
    ...
    def of_ds_set_calculate_values(self, list_res, base):
        ...
        for res in list_res:
            ...
            if brand := self.env["of.product.brand"].browse(res["brand_id"][0]):
    ...
    def get_single_product_variant(self):
        ...
        # en cas d'utilisation du configurateur de produit on doit retourner la variante ici directement
    ...
    def of_ds_import(self):
        ...
        for product in self:
            # on regarde avant tout s'il n'existe pas un produit avec la même marque et la même référence si c'est le
            # cas, on retourne le produit déjà existant
            if existing_product := self.env["product.template"].search(
                [
            ...
            if res := self.of_ds_read(base, [product_id], fields_to_import, check_fields=True):
                ...
                for r in res:
                    ...
                    for key in r:
                        if field := record._fields.get(key):
        ...
        for product in products:
            product.pack_line_ids = pack_line_dict.get(product.default_code, [])
        return products
    ...
    def of_ds_import_pack_line_ids(self, base, values):
        ...
        res.extend(Command.link(id_) for id_ in positive_ids)
        return res
