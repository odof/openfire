...
def intersect(hour_couple1, hour_couple2):
    ...
    hour_min = 0
    hour_max = 0
    # si hour_min du couple 2 est inférieur à celui du couple 1, alors hour_min = hour_couple2[0]
    # sinon hour_min = hour_couple1[0]
    if hour_couple2[0] > hour_couple1[0]:
        hour_min = hour_couple2[0]
    else:
        hour_min = hour_couple1[0]
    # si hour_min du couple 2 est inférieur à celui du couple 1, alors hour_min = hour_couple2[1]
    # sinon hour_min = hour_couple1[1]
    if hour_couple2[1] < hour_couple1[1]:
        hour_max = hour_couple2[1]
    else:
        hour_max = hour_couple1[1]
    # si hour_min > hour_max, ce n'est pas un créneau que l'on peut utiliser
    if hour_min > hour_max:
        hour_min, hour_max = (0, 0)
    ...
    def build_website_creneaux(self, mode='day'):
        ...
        for creneau_b in self.planning_ids.filtered(lambda c: c.disponible).\
            ...
            if mode == 'day':
                ...
                # on récupère les différent créneaux disponible en fonction des couples (heure_debut, heure_fin)
                # en comparant les créneaux de l'employé et de la tache
                hours_available = []
                for hour_couple in creneaux_day:
                    hours = intersect(hour_couple1=hour_couple, hour_couple2=(debut_tz.hour, fin_tz.hour))
                ...
                for hour_min, hour_max in hours_available:
                    ...
                    while i < amount:
                        key = creneau_b.date + '-%s' % creneau_count
                        if key not in creneau_dict:
                            start = hour_min + i * duree
                            end = hour_min + (i + 1) * duree
                            hour_start_td = timedelta(hours=start)
                            hour_start_str = str(hour_start_td)
                            hour_end_str = str(timedelta(hours=end))
                            creneau_dict[key] = {
                                'key': key,
                                'date': creneau_b.date,
                                'ids': [creneau_b.id],
                                'selected': False,
                                'distance_min': distance_prec,
                                'description': u"%sh%s - %sh%s" % (hour_start_str[:2], hour_start_str[3:5],
                                                                   hour_end_str[:2], hour_end_str[3:5]),
