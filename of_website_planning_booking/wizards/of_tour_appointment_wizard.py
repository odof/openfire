...
class OFTourAppointmentWizard(models.TransientModel):
    ...
    def _build_website_slots(self, lines):
        """Builds the slots displayed in the website, called from the controller"""
        ...
        slots_dict = {}
        for line in lines:
            # Des créneaux disponibles chevauchent souvent l'heure de midi, il faut les ajouter aux créneaux front
        ...
        update_vals = [Command.clear()]
        for k in slots_dict:
            slot = slots_dict[k]
            vals = {}
            vals["key"] = slot["key"]
            vals["name"] = format_date(k)
            vals["description"] = _("Morning") if k.endswith("morning") else _("Afternoon")
            vals["date"] = slot["date"]
            vals["selected"] = slot["selected"]
            vals["planning_ids"] = [Command.link(id_p) for id_p in slot["ids"]]
            update_vals.append(Command.create(vals))
...
class OFTourAppointmentLineWebsite(models.TransientModel):
    ...
    key = fields.Char()
    date = fields.Date()
    description = fields.Char(string="Slot info")
