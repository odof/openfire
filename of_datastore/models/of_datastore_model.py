...
class OFDatastoreModel(models.AbstractModel):
    ...
    def _read_prefetch(self, ids):
        ...
        res_base = {}
        for id in ids:
            base_id = -id // DATASTORE_IND
            record_id = -id % DATASTORE_IND
        ...
        for id in ids:
            ...
            DS_CACHE.set_prefetch(self.env.cr.dbname, self._name, id)

    def _read(self, field_names):
        positive_ids = [i for i in self._ids if i > 0]
        negative_ids = [i for i in self._ids if i < 0]
        res = super(OFDatastoreModel, self.browse(positive_ids))._read(field_names)
        if not res:
            res = []
        ...
        for id in negative_ids:
            if not DS_CACHE.is_prefetch(self.env.cr.dbname, self._name, id):
                to_prefetch.append(id)
        ...
        for field_name in field_names:
            if field_name not in fields_in_cache:
        ...
        for key in res_base:
            base = self.env["of.datastore.supplier"].browse(key)
            ds_res = self.of_ds_read(base, res_base[key], field_names, load=None, check_fields=True)
            if ds_res:
                res += ds_res

        for id in negative_ids:
            record = super().browse(id)
            self.push_record_ds_cache_to_cache(record, field_names)
        return res
    ...
    def of_ds_fields_to_fetch(self):
        # les champs dont on veut les données
        # par défaut, on retourne tous les champs avec prefetch à True
        if len(self.ids) > 0:
    ...
    def of_ds_fields_to_force_read(self):
        # les champs qui doivent être retourné dans le read à chaque appels
        # uniquement depuis le ds_cache
        return []
    ...
    def of_ds_search(self, base, args, offset, limit, order, count):
        ds = base.of_datastore_connect()
        ds_obj = base.of_datastore_get_model(ds, self._name)
        list_res = base.of_datastore_search(ds_obj, args, offset, limit, order, count)
    ...
    def of_ds_read(self, base, ids, fields, load=None, check_fields=False, no_cache=False):
        ...
        if check_fields:
            if no_cache:
                ...
                # pour éviter les appels en continue à la base distante pour avoir
                # les colonnes de l'objet, on le sauvegarde dans notre cache
                columns = DS_CACHE.get_column(self.env.cr.dbname, self._name)
                if columns:
                    ds_fields = columns
                else:
                    ds = base.of_datastore_connect()
        ...
        for id in old_ids:
            value = {}
            record = super().browse(-(id + base_index))
            for field in fields:
                value_field = record._fields.get(field)
                if value_field:
                    if field == "id":
                        value["id"] = -(id + base_index)
                    else:
    ...
    def of_ds_match(self, field, value, base_index, data):
        """
        field : le field odoo qui est à matcher
        value : la valeur à matcher
        base_index : L'index de la base distante
        data : les données du record dans lequel est la value
        """
        # on va chercher en fonction du model en cours et du field
        # comment retourner la data
        # Cela peut être juste une conversion simple
        # Cela peut-être une valeur locale équivalente à la valeur distante
        if not field:
            return False
        if field.type in [
            "boolean",
        ...
        if field.type == "many2one":
            if value:
                ...
                res = self.env[obj].of_ds_match_many2one(value, base_index, data)
                return res
        ...
        if field.type == "many2many":
            if value:
                obj = field.comodel_name
                res = self.env[obj].of_ds_match_many2many(value, base_index, data)
                return res
        ...
        if field.type == "one2many":
            if value:
                obj = field.comodel_name
                res = self.env[obj].of_ds_match_one2many(value, base_index, data)
                return res
            else:
                return []
    ...
    def of_ds_match_many2one(self, value, base_index, data):
        ...
        # cette méthode est à surcharger dans les modèles où l'on veut la donnée
        return False

    def of_ds_match_many2many(self, value, base_index, data):
        # par défaut, on retourne une liste vide
        # cette méthode est à surcharger dans les modèles où l'on veut la donnée
        return []
        ...
        for id in ids:
            ...
            for f in fields:
                ...
                if field and field.name != "id":
                    ...
                    if record._ids[0] not in field_cache and id not in res_ids:
                        res_ids.append(id)
