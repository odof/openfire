...
class OFCRMFunnelConversion(models.Model):
    ...
    sales_objective_comparison = fields.Char(
        string=u"Comparaison Objectif (%)",
        compute='_compute_sales_objective_comparison',
        compute_sudo=True,
        readonly=True,
    )
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        for line in res:
            if 'sales_objective_comparison' in fields:
                if (
                    'sales_total' in line
                    and line['sales_total'] is not None
                    and line.get('ordered_turnover_objective', False)
                ):
            ...
            if 'sales_total_comparison' in fields:
                if (
                    'sales_total' in line
                    and line['sales_total'] is not None
                    and line.get('previous_sales_total', False)
                ):
            ...
            if 'order_amount_rate' in fields:
                if 'sales_total' in line and line['sales_total'] is not None and line.get('quotation_amount', False):
                    line['order_amount_rate'] = (
                        '%.2f' % (round(100.0 * line['sales_total'] / line['quotation_amount'], 2))
                    ).replace('.', ',')
                else:
...
class OFCRMFunnelConversion2(models.Model):
    ...
    order_cart = fields.Char(string=u"Panier moyen", compute='_compute_order_cart', compute_sudo=True, readonly=True)
    activity_nb = fields.Integer(string=u"Nombre d'activités", readonly=True)
    quotation_amount = fields.Float(string=u"Montant devis", readonly=True)
    ...
    def _compute_order_margin_percent(self):
        for rec in self:
            if rec.sales_total > 0:
                rec.order_margin_percent = '%.2f' % (100 * (1 - (rec.sales_total - rec.order_margin) / rec.sales_total))
            else:
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        res = super(OFCRMFunnelConversion2, self).read_group(
            domain, fields.extend(['sales_total']), groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy
        )
        for line in res:
            if 'quotation_rate' in fields:
                if 'quotation_nb' in line and line['quotation_nb'] is not None and line.get('opportunity_nb', False):
                    line['quotation_rate'] = (
                        '%.2f' % (round(100.0 * line['quotation_nb'] / line['opportunity_nb'], 2))
                    ).replace('.', ',')
                else:
            ...
            if 'order_rate1' in fields:
                if 'order_nb' in line and line['order_nb'] is not None and line.get('quotation_nb', False):
                    line['order_rate1'] = (
                        '%.2f' % (round(100.0 * line['order_nb'] / line['quotation_nb'], 2))
                    ).replace('.', ',')
                else:
            ...
            if 'order_rate2' in fields:
                if 'order_nb' in line and line['order_nb'] is not None and line.get('opportunity_nb', False):
                    line['order_rate2'] = (
                        '%.2f' % (round(100.0 * line['order_nb'] / line['opportunity_nb'], 2))
                    ).replace('.', ',')
                else:
            ...
            if 'opportunity_cart' in fields:
                if 'sales_total' in line and line['sales_total'] is not None and line.get('opportunity_nb', False):
                    line['opportunity_cart'] = (
                        '%.2f' % (round(line['sales_total'] / line['opportunity_nb'], 2))
                    ).replace('.', ',')
                else:
            ...
            if 'quotation_cart' in fields:
                if 'sales_total' in line and line['sales_total'] is not None and line.get('quotation_nb', False):
                    line['quotation_cart'] = ('%.2f' % (round(line['sales_total'] / line['quotation_nb'], 2))).replace(
                        '.', ','
                    )
                else:
            ...
            if 'order_cart' in fields:
                if 'sales_total' in line and line['sales_total'] is not None and line.get('order_nb', False):
                    line['order_cart'] = ('%.2f' % (round(line['sales_total'] / line['order_nb'], 2))).replace('.', ',')
                else:
                    line['order_cart'] = "N/E"
            if 'order_margin_percent' in fields:
                if (
                    'sales_total' in line
                    and line['sales_total'] is not None
                    and 'order_margin' in line
                    and line['order_margin'] is not None
                    and line.get('sales_total', False)
                ):
                    line['order_margin_percent'] = (
                        '%.2f'
                        % (round(100 * (1 - (line['sales_total'] - line['order_margin']) / line['sales_total']), 2))
                    ).replace('.', ',')
                else:
