...
class OfInvoicedRevenueAnalysis(models.Model):
    ...
    landing_forecast_sales = fields.Char(
        string=u"Atterrissage CA prévisionnel",
        compute='_compute_landing_forecast_sales',
        compute_sudo=True,
        readonly=True,
    )

    invoiced_turnover_budget_gap = fields.Char(
        string=u"Écart au budget (€)", compute='_compute_invoiced_turnover_budget_gap', compute_sudo=True, readonly=True
    )
    ...
    invoiced_turnover_budget_comparison = fields.Char(
        string=u"Comparaison Budget (%)",
        compute='_compute_invoiced_turnover_budget_comparison',
        compute_sudo=True,
        readonly=True,
    )
    ...
    def _compute_invoiced_turnover_budget_gap(self):
        for rec in self:
            rec.invoiced_turnover_budget_gap = '%.0f' % (
                (rec.invoiced_total + rec.amount_to_invoice + rec.amount_to_invoice_intervention)
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        for line in res:
            ...
            if 'landing_forecast_sales' in fields_copy:
                if (
                    'invoiced_total' in line
                    and line['invoiced_total'] is not None
                    and 'amount_to_invoice' in line
                    and line['amount_to_invoice'] is not None
                    and 'amount_to_invoice_intervention' in line
                    and line['amount_to_invoice_intervention'] is not None
                ):
            ...
            if 'invoiced_turnover_budget_gap' in fields_copy:
                if (
                    ...
                    and 'amount_to_invoice_intervention' in line
                    and line['amount_to_invoice_intervention'] is not None
                    and 'invoiced_turnover_budget' in line
                    and line['invoiced_turnover_budget'] is not None
                ):
                    value = (
                        line['invoiced_total'] + line['amount_to_invoice'] + line['amount_to_invoice_intervention']
                    ) - line['invoiced_turnover_budget']
            ...
            if 'invoiced_total_comparison' in fields_copy:
                if (
                    'invoiced_total' in line
                    and line['invoiced_total'] is not None
                    and line.get('previous_invoiced_total', False)
                ):
            ...
            if 'invoiced_turnover_budget_comparison' in fields_copy:
                if (
                    'invoiced_total' in line
                    and line['invoiced_total'] is not None
                    and line.get('invoiced_turnover_budget', False)
                ):
