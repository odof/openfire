...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if (
            report.report_name == 'sale.report_saleorder'
            and self.user_has_groups('of_sale_report_setting.group_of_sale_report_print_attachment')
            and (res_ids and (isinstance(res_ids, int) or len(res_ids) == 1))
        ):
            ...
            for attach in product_attachments:
                if rendered_file := base64.b64decode(attach.datas):
