...
class GeoLocalizeAddressQuery(graphene.ObjectType):
    ...
    @staticmethod
    def resolve_geo_localize_address(root, info, street="", zzip="", city="", state="", country=""):
        env = info.context["env"]

        geo_obj = env["base.geocoder"]
        search = geo_obj.geo_query_address(street=street, zip=zzip, city=city, state=state, country=country)
