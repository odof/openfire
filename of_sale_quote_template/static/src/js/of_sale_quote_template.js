// On récupère le code complet de /website_quote/static/src/js/website_quotation.js
odoo.define('of_sale_quote_template.website_quote', function (require) {
'use strict';
...
if(!$('.o_website_quote').length) {
    ...
    ajax.loadXML('/of_sale_quote_template/static/src/xml/of_sale_quote_template.xml', QWeb);
    ...
    var AcceptModal = Widget.extend({
        ...
        // Ajout Openfire
        autoSignature: function (ev) {
            // Clean name text by removing characters that are not supposed to be used in a signature.
            var regexRemoveSpecialCharacters = /[^\w\u00E0-\u00FC-'" ]/g;
            var text = this.$('#name').val().replace(regexRemoveSpecialCharacters, '');

            // Get font data
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "https://fonts.gstatic.com/s/shadowsintolight/v6/clhLqOv7MXn459PTh0gXYMdQSYiIg2Yb25Hg13-ek1M.woff", false);
            xhr.overrideMimeType("text/plain; charset=x-user-defined");
            xhr.send(null);

            var font = base64Encode(xhr.responseText);

            var canvas = this.$("#signature").find('canvas')[0];
            var $svg = $(QWeb.render('website_quote.sign_svg_text', {
                width: canvas.width,
            ...
            var img = "data:image/svg+xml," + encodeURI($svg[0].outerHTML);
            ...
            image.onload = function () {
                ...
                self.drawTimeout = setTimeout(function () {
                    ...
                    var $canvas = self.$("#signature").find('canvas');
        ...
        submitForm: function(ev){
            ...
            if (action == 'accept') {
                ...
                // Ajout Openfire
                var signer_email = self.$("#email").val();
                var sign_date = self.$("#date").val();
                // Fin ajout Openfire
                ...
                // Ajout Openfire
                self.$('#signer_email').toggleClass('has-error', !signer_email);
                // Fin ajout Openfire
                self.$('#drawsign').toggleClass('panel-danger', is_empty).toggleClass('panel-default', !is_empty);
                if (is_empty || ! signer_name || ! signer_email){
                    setTimeout(function () {
...
odoo.define('of_sale_quote_template.payment_method', function (require) {
'use strict';
