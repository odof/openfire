...
class OfSaleQuoteTemplateLayoutCategory(models.Model):
    ...
    def _compute_depth(self):
        for category in self:
            if category.parent_id:
                category.depth = 1 + category.parent_id._compute_depth_recursive()
            else:
                category.depth = 0

    def _compute_depth_recursive(self):
        if self.parent_id:
            return 1 + self.parent_id._compute_depth_recursive()
        else:
            return 0
...
class OfSaleOrderLayoutCategory(models.Model):
    ...
    def get_color(self, model):
        ...
        new_rgb_int = [int(hex_value, 16) + (self.depth * 42) for hex_value in rgb_hex]
    ...
    def order_lines_layouted(self):
        ...
        if self.user_has_groups('of_sale_quote_template.group_of_advanced_sale_layout_category') \
            ...
            for index, layout_category in enumerate(of_layout_category_ids_sorted):
                new_dict[index] = {
                    'layout_category': layout_category,
                    'invoice_line_ids': self.invoice_line_ids.search([('of_layout_category_id', '=', layout_category.id)]),
                }
