...
class OfService(models.Model):
    ...
    in_progress_time = fields.Float(
        string=u"Temps de prise en compte (heures)",
        compute='_compute_in_progress_time',
        store=True,
        group_operator="avg",
    )
    ...
    @api.depends(
        'parc_installe_id.intervention_ids',
        'address_id.intervention_address_ids',
...
class OfPlanningIntervention(models.Model):
        ...
        for interv in self:
            ...
            interv.historique_rdv_ids = self.search(
                [
                    ...
                    '|',
                    ('parc_installe_id', '=', False),
                    ('parc_installe_id', '!=', interv.parc_installe_id.id),
                ]
            )
    ...
    def _onchange_address_id(self):
        ...
        if (
            self.address_id
            and self.parc_installe_id.site_adresse_id != self.address_id
            and not self._context.get('from_portal')
        ):
            parc_installe = False
            parc_obj = self.env['of.parc.installe']
            if not parc_obj.check_access_rights('read', raise_exception=False):
...
class OfParcInstalle(models.Model):
    ...
    def historique(self, intervention):
        ...
        return self.env['of.planning.intervention'].search(
            [('id', 'in', self.intervention_ids.ids), ('date', '<', intervention.date)], order="date DESC", limit=3
        )
