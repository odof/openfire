...
class OFServiceRequest(models.Model):
    ...
    def _compute_linked_equipment_ids(self):
        ...
        requests_use_equipment = self.filtered(lambda r: r.use_equipment and (r.address_id or r.partner_id))
        for request in requests_use_equipment:
            ...
            if request.address_id:
                domain += [
                    ...
                    ("customer_id", "=", request.address_id.id),
                ]
            if request.partner_id:
                domain[:0] = ["|"]
                domain += [("customer_id", "=", request.partner_id.id)]

            all_equipments = equipment_obj.search(domain) if domain else equipment_obj.browse()
            if not (
                equipments := all_equipments.filtered(
                    lambda e: e.site_address_id == request.address_id or e.customer_id == request.address_id
                )
            ):
                # If there is no equipment linked to the address we fallback to the partner
                equipments = all_equipments.filtered(lambda e: e.customer_id == request.partner_id)

            # And if only one equipment is found, we directly link it to the service request otherwise we keep the
            ...
            request.linked_equipment_ids = (
                ...
                if len(equipments) == 1
                else request.linked_equipment_ids
            )
