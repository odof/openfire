...
class OFServiceRequest(models.Model):
    ...
    def _compute_duration(self):
        ...
        for request in requests_with_equipment:
            if all(line.task_id for line in request.linked_equipment_ids):
                request.duration = sum(link.task_id.duration for link in request.linked_equipment_ids)
            else:
        ...
        if updated_links:
            ...
            for data in updated_links:  # [{"link": link, "old_values": old_values}, ...]
                ...
                if event_equipment_lnk := request_equipment_lnk.service_request_id.mapped(
                    ...
                    # Write the diff in the event equipment link linked to the service request
                    ...
                    event_equipment_lnk._post_event_message(data["old_values"], new_values)
    ...
    def _prepare_service_request_equipment_link_values_from_equipment(self, equipment):
        ...
        return {
            ...
            "equipment_report_tmpl_id": self.template_id.default_equipment_report_tmpl_id.id or False,
            "task_id": self.template_id and self.template_id.default_equipment_report_tmpl_id.task_id.id or False,
        }
