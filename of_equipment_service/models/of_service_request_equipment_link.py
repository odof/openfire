...
class OFServiceRequestEquipmentLink(models.Model):
    ...
    partner_id = fields.Many2one(comodel_name="res.partner", related="service_request_id.partner_id", string="Customer")
    address_id = fields.Many2one(comodel_name="res.partner", related="service_request_id.address_id", string="Address")
    ...
    def _post_request_message(self, old_values=None, new_values=None):
        """ """
        self.ensure_one()
        ...
        self.service_request_id.message_post(
            body=_(
                "Equipment details have been updated on the event for %(equipment)s.<br/>%(changes)s",
