...
class ProjectIssue(models.Model):
    ...
    def migrer_sav_di(self):
        ...
        if not self.env['ir.config_parameter'].sudo().get_param('of_migration_sav_di'):
            ...
            sav_avec_di = (
                service_obj.search([('sav_id', '!=', False)]).mapped('sav_id').filtered(lambda s: not s.is_migrated)
            )
...
WHERE os.sav_id = pi.id AND pi.id in %s""",
                    (sav_type.id, sav_avec_di._ids),
                )
...
ON CONFLICT DO NOTHING"""
            ...
            cr.execute(
                """UPDATE of_planning_intervention opi
