...
class OfPlanningIntervention(models.Model):
    ...
    def _compute_interval(self):
        for rdv in self:
            if not rdv.interv_before_id or not rdv.interv_before_id.address_id or not rdv.address_id:
                continue
            origine = rdv.interv_before_id.address_id
            arrivee = rdv.address_id
            ...
            if res and res.get('routes'):
                rdv.before_to_this = (float(res['routes'].pop(0)['duration']) / 60.0) / 60.0
...
class ResPartner(models.Model):
    ...
    intervention_partner_ids = fields.One2many('of.planning.intervention', string="RDVs Tech client", compute="_compute_rdvtech")
    intervention_address_ids = fields.One2many('of.planning.intervention', string="RDVs Tech adresse", compute="_compute_rdvtech")
