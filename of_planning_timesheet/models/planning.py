...
class OFPlanningIntervention(models.Model):
    ...
    def _compute_of_account_id(self):
        ...
        for intervention in self:
            ...
            if not account and intervention.order_id:
                account = intervention.order_id.project_id
            if not account:
                account = self.env.ref('of_planning_timesheet.default_analytic_project')
    ...
    def write(self, vals):
        ...
        if vals.get('employee_ids'):
            ...
            for intervention in self:
                intervention_lines = intervention.of_analytic_line_ids.filtered(
                    lambda x: x.of_type == 'intervention')
                user_to_add = users - intervention_lines.mapped('user_id')
                line_to_delete = intervention_lines.filtered(lambda x: x.user_id not in users)
