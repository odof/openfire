...
class OFParcInstalle(models.Model):
    ...
    type_garantie = fields.Selection(
        selection=[('initial', u"Initiale"), ('extension', u"Extension"), ('expired', u"Expirée")],
    ...
    site_adresse_id = fields.Many2one(
        comodel_name='res.partner',
        string=u"Site installation",
        required=False,
        domain="['|',('parent_id','=',client_id),('id','=',client_id)]",
        ondelete='restrict',
    )
    revendeur_id = fields.Many2one(comodel_name='res.partner', string=u"Revendeur", required=False, ondelete='restrict')
    ...
    installateur_adresse_id = fields.Many2one(
        comodel_name='res.partner',
        string=u"Adresse installateur",
        required=False,
        domain="['|',('parent_id','=',installateur_id),('id','=',installateur_id)]",
        ondelete='restrict',
    )
    ...
    @api.depends(
        ...
        'client_id.precision',
        'site_adresse_id',
        'site_adresse_id.geo_lat',
        'site_adresse_id.geo_lng',
        'site_adresse_id.precision',
    )
    ...
    def onchange_date_fin_garantie(self):
        if self.date_fin_garantie and self.date_fin_garantie < fields.Date.today():
            ...
            self.date_fin_garantie and self.date_fin_garantie >= fields.Date.today() and self.type_garantie == 'expired'
        ):
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if self._context.get('partner_id_no_serie_puce'):
            ...
            res += (
                super(OFParcInstalle, self).name_search(name, [('client_id', '!=', client_id)], operator, limit) or []
            )
            return res
        if self._context.get('address_prio_id'):
            ...
            args = args or []
            res = (
                super(OFParcInstalle, self).name_search(
                    name, args + [['site_adresse_id', '=', address_id]], operator, limit
                )
                or []
            )
            limit = limit - len(res)
    ...
    def action_creer_sav(self):
        ...
        if active_ids:
            ...
            if parc_installe.client_id:
                res['context'] = {
                    'default_partner_id': parc_installe.client_id.id,
